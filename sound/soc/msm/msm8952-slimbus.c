...
int get_audiowizard_state(void) {
	...
	if (audiowizard_sdev.dev)
		state = switch_get_state(&audiowizard_sdev);
	else
		state = -ENODEV;
...
struct msm8952_asoc_mach_data {
	...
	struct delayed_work cap_noise_dwork;
...
static int speaker_media_vol_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: speaker_media_vol_get = %d\n", __func__,
						spk_media_vol);
	ucontrol->value.integer.value[0] = spk_media_vol;
	return 0;
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (isSpkOn) {
		...
		pr_info("kcontrol: notify cap sensor threshold vol = %d\n",
				 spk_media_vol);
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Speaker Media Volume", SND_SOC_NOPM, 0, 15, 0,
			speaker_media_vol_get, speaker_media_vol_put),
...
void msm_quat_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	pr_info("%s: notify cap threshold to Normal\n", __func__);
...
int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	mutex_lock(&spk_playing_lock);
	isSpkOn = true;
	pr_info("%s: notify cap threshold quat i2s start\n", __func__);
	notify_speaker_status(1, spk_media_vol);
	mutex_unlock(&spk_playing_lock);
	return ret;
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		pr_err("%s: failed to register audiowizrd switch device",
			__func__);
