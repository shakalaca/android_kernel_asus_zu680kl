...
extern void ohio_set_power_supply(struct power_supply *psy);
extern int ohio_get_id_state(void);

static bool aca_id_turned_on;
static bool legacy_power_supply;
static int retry_cdp = 0;
extern int asus_force_reset_as_CDP(void);
...
static bool g_skippid = false;
//ASUS_BSP--- Landice "[ZE500KL][USBH][NA][Spec] Skip set ID events when screen off"
#ifdef CONFIG_DUAL_ROLE_USB_INTF
static int g_host_charging_mode = 1;
#else
...
bool msm_otg_id_state(void) {

	if(!the_msm_otg) {
		pr_err("msm otg not ready\n");
		return true;
	}

	return test_bit(ID, &the_msm_otg->inputs);
}
...
{
	if (g_host_charging_mode == 1)
		seq_printf(s, "host charging\n");
	else
		seq_printf(s, "error\n");
	return 0;
}
...
static void asus_otg_fb_late_resume(void)
{
	...
	if (motg->host_mode && g_suspend_delay_work_run) {
		...
		g_suspend_delay_work_run = 0;
	} else if (!motg->host_mode && g_skippid){
		queue_delayed_work(motg->otg_wq, &motg->id_status_work, 0);
	}
	printk("%s()---\n", __func__);
}
...
static void msm_otg_notify_host_mode(struct msm_otg *motg, bool host_mode)
{
	...
	if (legacy_power_supply) {
		...
		/*motg->host_mode = host_mode; mvoe to id interrupt handler*/
			...
			unsigned long action, void *priv)
{
	...
	if (action == USB_DEVICE_ADD || action == USB_DEVICE_CONFIG) {
		...
		printk("[usb_otg] USB Device Removed\n");
		g_keep_power_on = 0;
	}
...
static void dm_pull_function(struct msm_otg *motg)
{
	struct usb_phy *phy = &motg->phy;
...
#define CDP_RETRY_TIME			(5000 * HZ/1000)
	...
	case USB_CHG_STATE_DETECTED:
		...
		if (retry_cdp == 1) {
			if (motg->chg_type == USB_CDP_CHARGER) {
				/* notify charger dcp */
				printk("[usb_otg] CDP is detect\n");
				motg->is_ext_chg_dcp = false;
				msm_chg_block_off(motg);
				asus_force_reset_as_CDP();
				queue_work(motg->otg_wq, &motg->sm_work);
			} else {
				printk("[usb_otg] Other type is detect\n");
				if (!retry) {
					retry = true;
					msm_chg_block_off(motg);
					motg->chg_state = USB_CHG_STATE_UNDEFINED;
					motg->chg_type = USB_INVALID_CHARGER;
					delay = CDP_RETRY_TIME;
					break;
				}
				msm_chg_block_off(motg);
				motg->chg_type = USB_DCP_CHARGER;
...
void asus_phy_chg_detect(void){

	printk("[usb_otg] Other type retry\n");
	retry_cdp = 1;
}
EXPORT_SYMBOL(asus_phy_chg_detect);
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			...
			if (pdata->pmic_id_irq) {
				...
				if (ohio_get_id_state())
					set_bit(ID, &motg->inputs);
				else
					clear_bit(ID, &motg->inputs);
				/*PIMC notify id state by usb power supply*/
				/*
				if (motg->id_state)
					set_bit(ID, &motg->inputs);
				else
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			msm_otg_dbg_log_event(&motg->phy,
					...
					get_pm_runtime_counter(otg->phy->dev),
					motg->pm_done);
			printk("PM RUNTIME Counter = %d\n", get_pm_runtime_counter(otg->phy->dev));
			i = get_pm_runtime_counter(otg->phy->dev);
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
		...
		if(g_reverse_charger_mode) {
			...
			} else if ((pc & PORTSC_CSC) && !(pc & PORTSC_CCS)) {
				printk("[usb_otg] OTG unplug\n");
				dm_pull_function(motg);
			}
		}
...
static void msm_otg_set_vbus_state(int online)
{
	struct msm_otg *motg = the_msm_otg;
	static bool init;
	struct usb_otg *otg = the_msm_otg->phy.otg;
	int i = 0;
...
out:
	...
	if (motg->is_ext_chg_dcp) {
		if (test_bit(B_SESS_VLD, &motg->inputs)) {
			...
			cancel_delayed_work_sync(&motg->chg_work);
			printk("PM RUNTIME Counter = %d in BSV clear DCP\n", get_pm_runtime_counter(otg->phy->dev));
			i = get_pm_runtime_counter(otg->phy->dev);
			while (i > 0){
				pm_runtime_put_noidle(otg->phy->dev);
				i--;
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	if (test_bit(B_SESS_VLD, &motg->inputs) && !g_host_charging_mode) {
		if (init) {
			printk("[usb_otg] Ignore ID at vbus exist (%d)\n", motg->id_state);
			return;
		}
	}
	...
	if (motg->err_event_seen)
		return;
//ASUS_BSP+++ "[ZE500KL][USBH][NA][Spec] Skip set ID events when screen off"
	if(motg->otg_mode == USB_AUTO && !(g_screen_off && !motg->id_state)) {
		g_skippid = false;
		printk("[usb_otg] %s, otg_mode = %d\n", __func__, motg->otg_mode);
	} else {
		g_skippid = true;
		printk("[usb_otg] %s, otg_mode = %d, skip ID detection (%d)(%d)\n", __func__, motg->otg_mode, motg->id_state, g_screen_off);
		return;
	}
//ASUS_BSP--- "[ZE500KL][USBH][NA][Spec] Skip set ID events when screen off"
	...
	if (motg->id_state) {
		...
		if (!test_and_set_bit(ID, &motg->inputs) || g_host_none_mode) {
			printk("[usb_otg]ID set\n");
			msm_otg_dbg_log_event(&motg->phy, "ID SET",
					motg->inputs, motg->phy.state);
			work = 1;
			motg->host_mode = false;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_USB_OTG:
		...
		queue_delayed_work(motg->otg_wq, &motg->id_status_work, g_id_delay);
		break;
		...
		if (motg->chg_type != USB_INVALID_CHARGER) {
			...
			if (retry_cdp == 1) {
				if (motg->chg_state == USB_CHG_STATE_UNDEFINED) {
					printk("[usb_otg] chg detect work\n");
					msm_chg_detect_work(&motg->chg_work.work);
				}
			} else
				motg->chg_state = USB_CHG_STATE_DETECTED;
					...
					struct msm_otg *motg)
{
	...
	ohio_set_power_supply(&motg->usb_psy);
...
msm_otg_ext_chg_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSM_USB_EXT_CHG_BLOCK_LPM:
		...
		pr_info("%s: LPM block request %d && type=%d\n", __func__, val, motg->chg_type);
