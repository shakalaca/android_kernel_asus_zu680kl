...
/* vibrator level set */
static int mercury_vmax_array[] = {
	1044, 1276, 1392, 1624, 1856, 1972, 1972
};
...
/* sysfs store for vibrator vmax level setting */
static ssize_t qpnp_vmax_level_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	if (value < 0 || value > sizeof(mercury_vmax_array)/sizeof(int)) {
		pr_err("[Vibrator] config vmax_level error value: %d\n", value);
		return -EINVAL;
	}

	hap->vmax_mv = mercury_vmax_array[value];
	pr_info("[Vibrator] vmax: %d, level: %d\n", hap->vmax_mv, value);

	if (qpnp_hap_vmax_config(hap))
		pr_err("[Vibrator] config vmax_level failed...\n");
...
/* sysfs store for setting the vibrator strength of device */
static ssize_t qpnp_vmax_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	hap->vmax_mv = value;
	if (qpnp_hap_vmax_config(hap))
		pr_err("[Vibrator] vmax_store failed...\n");
...
/* sysfs show function for vmax_mv value */
static ssize_t qpnp_vmax_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	/* approximate value */
	mv = (val >> QPNP_HAP_VMAX_SHIFT) * QPNP_HAP_VMAX_MIN_MV;
