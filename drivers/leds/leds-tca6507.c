...
static int EN_gpio;
static int tca6507_probe_done = 0;
...
struct tca6507_chip *g_tca6507_chip;
...
static void set_times(struct tca6507_chip *tca, int bank)
{
	...
	pr_info("LED: Chose on  times %d(%d) %d(%d) for %dms\n", c1, time_codes[c1],
	...
	pr_info("LED: Chose off times %d(%d) %d(%d) for %dms\n", c1, time_codes[c1],
...
tca6507_led_dt_init(struct i2c_client *client)
{
	...
	EN_gpio = of_get_named_gpio_flags(np,
		"en_gpio", 0, &flags);
...
static ssize_t tca6507_leds_on_set(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	...
	if (buf[0] == '6') {
		tca->leds[5].led_cdev.brightness_set(&tca->leds[5].led_cdev, 0);
		tca->leds[6].led_cdev.brightness_set(&tca->leds[6].led_cdev, 0);
	}
	else if (buf[0] == '5') {
		 tca->leds[6].led_cdev.brightness_set(&tca->leds[6].led_cdev, 255);
	}
	else if (buf[0] == '4') {
                tca->leds[5].led_cdev.brightness_set(&tca->leds[5].led_cdev, 255);
        } else if (buf[0] == '3') {
                tca->leds[0].led_cdev.brightness_set(&tca->leds[0].led_cdev, 127);
                tca->leds[1].led_cdev.brightness_set(&tca->leds[1].led_cdev, 127);
        }
	else if (buf[0] == '2') {
                tca->leds[0].led_cdev.brightness_set(&tca->leds[0].led_cdev, 0);
                tca->leds[1].led_cdev.brightness_set(&tca->leds[1].led_cdev, 255);
	}
	else if(buf[0] == '1') {
		tca->leds[0].led_cdev.brightness_set(&tca->leds[0].led_cdev, 255);
		tca->leds[1].led_cdev.brightness_set(&tca->leds[1].led_cdev, 0);
	}
	else if(buf[0] == '0') {
		tca->leds[0].led_cdev.brightness_set(&tca->leds[0].led_cdev, 0);
		tca->leds[1].led_cdev.brightness_set(&tca->leds[1].led_cdev, 0);
...
static ssize_t tca6507_leds_blink_set(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
        struct i2c_client * client = to_i2c_client(dev);
        struct tca6507_chip *tca =
                i2c_get_clientdata(client);
	unsigned long delay_on = time_codes[1], delay_off = time_codes[10];

	if (buf[0] == '3') {
		gpio_set_value(EN_gpio, 1);
                tca->reg_set = 0x7f;
                tca->leds[0].led_cdev.blink_set(&tca->leds[0].led_cdev, &delay_on, &delay_off);
		tca->leds[1].led_cdev.blink_set(&tca->leds[1].led_cdev, &delay_on, &delay_off);
	}
	else if (buf[0] == '2') {
		gpio_set_value(EN_gpio, 1);
                tca->reg_set = 0x7f;
...
static ssize_t tca6507_led_blink_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	unsigned long blinking;
	unsigned long pattern_flash = time_codes[4];
	unsigned long pattern1_delay = time_codes[12], pattern2_delay = time_codes[10];
	...
	pr_info("LED: %s setting blink mode: %d\n", cdev->name, (int)blinking);
	if (blinking == 0)
		cdev->brightness_set(cdev, 0);
	else if (blinking == 2)
		cdev->blink_set(cdev,  &pattern2_delay, &pattern2_delay);
	else {
		if (!strcmp(cdev->name, "green"))
			cdev->blink_set(cdev,  &pattern_flash, &pattern2_delay);
		else if (!strcmp(cdev->name, "red"))
			cdev->blink_set(cdev,  &pattern_flash, &pattern1_delay);
		else
			pr_err("undefined behavior of blinking\n");
	}
	return len;
}

static DEVICE_ATTR(tca6507_leds_on, S_IRUGO | S_IWUSR, NULL, tca6507_leds_on_set);
static DEVICE_ATTR(tca6507_leds_blink, S_IRUGO| S_IWUSR, NULL, tca6507_leds_blink_set);

static DEVICE_ATTR(blink, 0664, NULL, tca6507_led_blink_store);

void tca6507_set_VK_brightness(enum led_brightness brightness)
{
	struct tca6507_chip *chip = g_tca6507_chip;
	if (tca6507_probe_done) {
		chip->leds[5].led_cdev.brightness_set(&chip->leds[5].led_cdev, brightness);
		chip->leds[6].led_cdev.brightness_set(&chip->leds[6].led_cdev, brightness);
	}
}
EXPORT_SYMBOL(tca6507_set_VK_brightness);

static struct attribute *tca6507_attributes[] = {
	&dev_attr_tca6507_leds_on.attr,
	&dev_attr_tca6507_leds_blink.attr,
	NULL
};

static struct attribute *tca6507_led_attributes[] = {
	&dev_attr_blink.attr,
	NULL
};

static const struct attribute_group tca6507_group = {
	.attrs = tca6507_attributes,
};

static const struct attribute_group tca6507_led_attr_group = {
	.attrs = tca6507_led_attributes,
};
		...
		const struct i2c_device_id *id)
{
	...
	for (i = 0; i < NUM_LEDS; i++) {
		...
		if (pdata->leds.leds[i].name && !pdata->leds.leds[i].flags) {
			...
			if (err < 0)
				goto exit;
			err = sysfs_create_group(&l->led_cdev.dev->kobj, &tca6507_led_attr_group);
			if (err) {
	...
	if (gpio_is_valid(EN_gpio)) {
                err = gpio_request(EN_gpio, "tca6507 enable");
                if (err) {
                        pr_err("request for tca6507 enable gpio failed!\n");
			goto exit;
                }
		err = gpio_direction_output(EN_gpio, 0);
		if (err) {
			pr_err("tca6507 enable gpio set direction failed\n");
			goto exit;
		}
		pr_notice("tca6507 enable gpio: %d default set low.\n", EN_gpio);
        } else {
                pr_err("load enable gpio from dt failed!\n");
		goto exit;
        }

	err = sysfs_create_group(&client->dev.kobj, &tca6507_group);
	if (err) {
		pr_err("unable to create the sysfs\n");
	}
