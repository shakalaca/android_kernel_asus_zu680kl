...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	if (iris2_get_id() == 0x1) {
		if (time_to_end_of_vbl_ns) {
			if (current_line < pinfo->lcdc.v_pulse_width + pinfo->lcdc.v_back_porch)
				*time_to_end_of_vbl_ns = time_of_line * (pinfo->lcdc.v_pulse_width
									 + pinfo->lcdc.v_back_porch
									 - current_line);
			else if (current_line > total_line - pinfo->lcdc.v_front_porch)
				*time_to_end_of_vbl_ns = time_of_line * (total_line - current_line
									 + pinfo->lcdc.v_pulse_width
									 + pinfo->lcdc.v_back_porch);
			else
				*time_to_end_of_vbl_ns = 0;
		}
	}
#endif
