...
static struct smb1351_charger *g_smb1351_chip;
...
static int chargerIC_status_get(void *data, u64 *val)
{
	struct smb1351_charger *chip = data;
	*val = smb1351_enable_volatile_writes(chip) ? 0 : 1;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(chargerIC_status_debugfs_ops, chargerIC_status_get, NULL,
	"%llu\n");
...
static int asus_dump_regs(struct seq_file *m, void *data)
{
	...
	for (addr = 0; addr <= LAST_CNFG_REG; addr++) {
		rc = smb1351_read_reg(chip, addr, &reg);
		if (rc)
			seq_printf(m, "Couldn't read 0x%02x rc = %d\n", addr, rc);
		else
			seq_printf(m, "0x%02x = 0x%02x\n", addr, reg);
	}
...
static int create_debugfs_entries(struct smb1351_charger *chip)
{
	...
	if (!chip->debug_root) {
		...
		ent = debugfs_create_file("chargerIC_status", S_IFREG | S_IRUGO,
					chip->debug_root, chip,
					&chargerIC_status_debugfs_ops);
		if (!ent)
				pr_err("Couldn't create chargerIC_status debug file\n");

		ent = debugfs_create_file("dump_reg", S_IFREG | S_IRUGO,
					chip->debug_root, chip,
					&asus_dump_debugfs_ops);
		if (!ent)
				pr_err("Couldn't create dump reg debug file\n");
	}
	return 0;
}
...
void asus_smb1351_boost_enable(int enable, bool high_current, bool polling)
{
	struct smb1351_charger *chip;
	chip = g_smb1351_chip;
	...
	if (!polling)
		...
		smb1351_masked_write(chip, OTG_TLIM_CTRL_REG, OTG_OC_LIMIT_MASK, 0x4);
	if (enable) {
		// Set OTG to I2C control, 09h[7:6] = "10"
		smb1351_masked_write(chip, OTG_USBIN_AICL_CTRL_REG, OTG_ID_PIN_CTRL_MASK, 0x80);
		// Enable OTG Function by command, 32h[0] = "1"
		smb1351_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT, CMD_OTG_EN_BIT);
		if (high_current) {
			// Set OTG current limit = 1800 mA, 0Ah[3:2] = "00"
			smb1351_masked_write(chip, OTG_TLIM_CTRL_REG, OTG_OC_LIMIT_MASK, 0x0);
		} else {
			// Set OTG current limit = 1000 mA, 0Ah[3:2] = "11"
			smb1351_masked_write(chip, OTG_TLIM_CTRL_REG, OTG_OC_LIMIT_MASK, 0xC);
		}
	} else {
		// Disable OTG, 32h[0] = "0"
		smb1351_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT, 0);
	}
}
EXPORT_SYMBOL(asus_smb1351_boost_enable);
...
void asus_Dual_Enable(int usbin_cur, int fastchg_cur) {
	struct smb1351_charger *chip;
	int rc;

	pr_info("Dual Enabled: usbin_cur: %d mA, fastchg_cur: %d\n", usbin_cur, fastchg_cur);
	chip = g_smb1351_chip;
	smb1351_enable_volatile_writes(chip);
	...
	if (rc)
		pr_err("set usbin non-suspend failed\n");
	chip->usb_suspended_status = 0;
	// Set IUSB_IN, 00h[3:0]
	rc = smb1351_set_usb_chg_current(chip, usbin_cur);
	if (rc)
		pr_err("set usbin charging current = %d failed\n", usbin_cur);
	// Set fast charge current, 00h[7:4]
	rc = smb1351_fastchg_current_set(chip, fastchg_cur);
	if (rc)
		pr_err("set charging current = %d failed\n", fastchg_cur);

	// set Charger Enable, 06h[6:5] = "10"
	rc = smb1351_masked_write(chip, CHG_PIN_EN_CTRL_REG,
					EN_PIN_CTRL_MASK, 0x40);
	if (rc)
		pr_err("set charger enable failed\n");
}
EXPORT_SYMBOL(asus_Dual_Enable);

void asus_smb1351_JEITA_rule(void)
{
	struct smb1351_charger *chip;
	int rc;
	chip = g_smb1351_chip;
	// set Hard Hot Limit = 72 Deg. C, 0Bh[5:4] = "11"
	rc = smb1351_masked_write(chip, HARD_SOFT_LIMIT_CELL_TEMP_REG,
				HARD_LIMIT_HOT_TEMP_ALARM_TRIP_MASK, 0x30);
	if (rc)
		pr_err("set hot limit = 72 degree failed\n");
	// set Soft colde limit behavior = no response, 07h[3:2] = "00"
	rc = smb1351_masked_write(chip, THERM_A_CTRL_REG,
				SOFT_COLD_TEMP_LIMIT_MASK, 0x0);
	if (rc)
		pr_err("set soft limit = no response failed\n");
	// set Soft Hot Limit = no response, 07h[1:0] = "00"
	rc = smb1351_masked_write(chip, THERM_A_CTRL_REG,
				SOFT_HOT_TEMP_LIMIT_MASK, 0x0);
	if (rc)
		pr_err("set soft hot limit = no response failed\n");

}
EXPORT_SYMBOL(asus_smb1351_JEITA_rule);

void asus_smb1351_initial_setting(void)
{
	struct smb1351_charger *chip;
	int rc;
	chip = g_smb1351_chip;
	...
	if (rc)
		...
		chip->usb_suspended_status = true;
	smb1351_masked_write(chip, VFLOAT_REG, VFLOAT_MASK, 0x2C);
	smb1351_masked_write(chip, CMD_INPUT_LIMIT_REG,
				CMD_INPUT_CURRENT_MODE_BIT | BIT(0), 0x9);
	smb1351_masked_write(chip, VARIOUS_FUNC_REG, AICL_EN_BIT, 0x0);
	smb1351_masked_write(chip, WDOG_SAFETY_TIMER_CTRL_REG, WDOG_TIMER_EN_BIT, 0x1);
}
EXPORT_SYMBOL(asus_smb1351_initial_setting);

void asus_smb1351_enable_wdt(bool enable)
{
	struct smb1351_charger *chip= g_smb1351_chip;
	if (enable)
		smb1351_masked_write(chip, WDOG_SAFETY_TIMER_CTRL_REG, WDOG_TIMER_EN_BIT, 0x1);
	else
		smb1351_masked_write(chip, WDOG_SAFETY_TIMER_CTRL_REG, WDOG_TIMER_EN_BIT, 0x0);
}
EXPORT_SYMBOL(asus_smb1351_enable_wdt);
				...
				const struct i2c_device_id *id)
{
	...
	chip->asus_int_gpio= of_get_named_gpio_flags(node,
		"irq_gpio", 0, &flags);
	if (gpio_is_valid(chip->asus_int_gpio)) {
                rc = gpio_request(chip->asus_int_gpio, "smb1351_int_gpio");
                if (rc) {
                        pr_err("requset smb1351 irq gpio failed\n");
                        return rc;
                }
                rc = gpio_direction_input(chip->asus_int_gpio);
                if (rc) {
                        pr_err("smb1351 irq gpio set direction failed\n");
                        return rc;
                }
        }
...
static void asus_smb1351_charger_shutdown(struct i2c_client *client)
{
	struct smb1351_charger *chip = i2c_get_clientdata(client);
	smb1351_enable_volatile_writes(chip);
...
static struct i2c_driver smb1351_charger_driver = {
	...
	.shutdown	= asus_smb1351_charger_shutdown,
