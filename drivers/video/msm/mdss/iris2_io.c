...
static int iris2_io_probe(struct platform_device *pdev)
{
	...
	if((hw_id == 0)||(hw_id == 1)){
		...
		pw_iris2_chip_version = IRIS2_26;
	}

	printk(KERN_CRIT "[DISPLAY] hw_id = %d, pw_iris_id = %d, pw_iris2_chip_version = %d\n", hw_id, pw_iris2_id, pw_iris2_chip_version);
