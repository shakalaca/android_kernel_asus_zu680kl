...
#endif
{
    ...
    //ret = gpio_direction_output(pn5xx_dev->ven_gpio, 1);
    if (ret < 0) {
    ...
    if (gpio_is_valid(pn5xx_dev->firm_gpio)) {
        ...
        //ret = gpio_direction_output(pn5xx_dev->firm_gpio, 1);
        if (ret < 0) {
    ...
    //ret = gpio_direction_output(pn5xx_dev->ese_pwr_gpio, 1);
    //if (ret < 0) {
