...
#if defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
extern void iris2_io_resume(void);
extern void iris2_io_suspend(void);
...
static bool bl_wled_enable = true;
extern int cabc;
				...
				struct mdss_rect *roi, int unicast)
{
	...
	pr_debug("%s: ndx=%d col_addr[0x%x, 0x%x, 0x%x, 0x%x], pag_addr[0x%x, 0x%x, 0x%x, 0x%x]\n",
			__func__, ctrl->ndx,
...
#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP) || defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
	if(ctrl_pdata->bklt_ctrl == BL_WLED && cResume == 1 && FrameCount < 2)
	{
		int DelayCount=0;
		for(DelayCount = 0; DelayCount < 50; DelayCount++)
		{
			...
			if(FrameCount > 1)
			{
				cResume = 0;
				FrameCount = 0;
				pr_debug("DealyCount = %d\n",DelayCount);
				pr_debug("Delay for Frame ready is done!\n");
				break;
			}
		}
	} else if (ctrl_pdata->bklt_ctrl == BL_WLED && cResume == 1 && FrameCount >= 2) {
		pr_debug("FrameCount is greater than 2, reset cResume and FrameCount\n");
		cResume = 0;
		FrameCount = 0;
	}
#endif
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	u8 powermode = 0x0;
	cResume =1;
	pr_debug("mdss_dsi_panel_on cResume = %d , FrameCount= %d\n",cResume,FrameCount);
#endif
...
#if defined(TARGET_PROJECT_ZU680KL_DISPLAY)
	// for zu680 project, dsi1 is used for DP display
	if(ctrl->ndx == 1){
	...
	*/
	...
	if (iris2_get_id() == 0x1) {
		if ((pinfo->mipi.dms_mode == DYNAMIC_MODE_SWITCH_IMMEDIATE) &&
				...
				;//8976_TODO, add post dms_on_cmds.
		// configure iris by dsi0
		if (ctrl->ndx == DSI_CTRL_LEFT) {
			...
			iris_add8lines_set(ctrl);

			iris_panel_on_start(ctrl);
	#ifdef IRIS_BYPASS
			iris_mipi_bypass_ex(ctrl);

			iris_cmds(ctrl, PANEL_ON_CMDS, ctrl->ndx);
			iris_mipi_mcu(ctrl);
	#else //IRIS_BYPASS
			iris_cmds(ctrl, PANEL_ON_CMDS, ctrl->ndx);
	#endif //IRIS_BYPASS
...
//			iris_mcuclk_divider_change(ctrl, 1);
	} else {
		if ((pinfo->mipi.dms_mode == DYNAMIC_MODE_SWITCH_IMMEDIATE) &&
				(pinfo->mipi.boot_mode != pinfo->mipi.mode))
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (iris2_get_id() == 0x1) {
		...
		if (ctrl->ndx == DSI_CTRL_LEFT) {
			...
			iris_mipi_bypass_ex(ctrl);
			iris_cmds(ctrl, PANEL_OFF_CMDS, ctrl->ndx);

			iris_mipi_mcu(ctrl);
			iris_panel_off_finish(ctrl);
		} else if (ctrl->ndx == DSI_CTRL_RIGHT) {
			iris_mipi_bypass_ex(ctrl);
			iris_cmds(ctrl, PANEL_OFF_CMDS, ctrl->ndx);
		}
	...
	int enable)
{
	...
	cResume =0;
	FrameCount=0;
	pr_debug("mdss_dsi_panel_off cResume = %d , FrameCount = %d\n",cResume,FrameCount);
#endif
...
#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP) || defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
static void mdss_dsi_parse_iris_params(struct device_node *np,
					struct mdss_panel_info *panel_info)
{
	int rc;
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (pinfo->pdest== 0) {
		Tcon_frame_rate = pinfo->mipi.frame_rate;
		printk("Display check panel frame rate ,Tcon_frame_rate =%d %d \n",pinfo->mipi.frame_rate,Tcon_frame_rate);
	}
	...
	pr_info("%s: is_dba_panel=%d\n", __func__, pinfo->is_dba_panel);

#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP) || defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
	if (iris2_get_id() == 0x1) {
		if (pinfo->pdest == DISPLAY_1)
			mdss_dsi_parse_iris_params(np, pinfo);	// only configure the param at dsi0
		mdss_dsi_parse_iris_mipi(np, pinfo->mipi.mode, pinfo->pdest);
	}
#endif
	return 0;
...
static void mdss_dsi_set_prim_panel(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pr_info("%s: is_prim_panel=%d\n", __func__, pinfo->is_prim_panel);
}

static int lcd_unique_id_show(struct seq_file *s, void *p)
{
	/* no unique id for jdi lpm068a253a,
	   so always return 0 */
	seq_printf(s, "%d\n", 0);
	return 0;
}

static int lcd_unique_id_open(struct inode *inode, struct file *file)
{
	return single_open(file, lcd_unique_id_show, NULL);
}

static const struct file_operations lcd_unique_id_proc_fops = {
	.open           = lcd_unique_id_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
