...
#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP)
#include "mdss_dsi_iris_core.h"
#include "iris2_io.h"
#elif defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	if (mdss_mdp_display_wakeup_time(mdp5_data->ctl, &wakeup_time, NULL))
#else
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	// iris
	if (iris2_get_id() == 0x1 && mfd->panel_info->type == MIPI_VIDEO_PANEL && mfd->panel_info->is_prim_panel) {
		iris_copy_meta(mfd);
		iris_frc_repeat(mfd);
		iris_calc_nrv(ctl);
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
static ssize_t vtotal_adjust_sysfs_read(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret;
	struct mdss_panel_data *pdata;
	...
	if (!mdp5_data->ctl || (mdp5_data->ctl->power_state == MDSS_PANEL_POWER_OFF))
		return 0;
	...
	ret = snprintf(buf, PAGE_SIZE, "%d\n",
		       pdata->panel_info.vtotal_adjust);
	pr_debug("%s: '%d'\n", __func__,
		pdata->panel_info.vtotal_adjust);
...
static ssize_t vtotal_adjust_sysfs_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int vadj, rc = 0;
	struct mdss_panel_data *pdata;
	...
	if (vadj == pdata->panel_info.vtotal_adjust) {
		pr_debug("%s: FPS is already %d\n",
			__func__, vadj);
		return count;
	}

	rc = mdss_mdp_video_timegen_vtotal_adjust(mdp5_data, vadj);
	if (rc)
		return rc;
	pdata->panel_info.vtotal_adjust = vadj;
	return count;
}

static DEVICE_ATTR(vtotal_adjust, S_IRUGO | S_IWUGO, vtotal_adjust_sysfs_read,
	vtotal_adjust_sysfs_write);

static struct attribute *vtotal_adjust_fs_attrs[] = {
	&dev_attr_vtotal_adjust.attr,
	NULL,
};

static struct attribute_group vtotal_adjust_fs_attrs_group = {
	.attrs = vtotal_adjust_fs_attrs,
};
#endif
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	vsync_count = mdp5_data->vsync_count;

	pr_debug("fb%d vsync=%llu, vsync_count=%llu", mfd->index, vsync_ticks, vsync_count);
	ret = scnprintf(buf, PAGE_SIZE, "VSYNC=%llu COUNT=%llu", vsync_ticks, vsync_count);
#else
...
static int msmfb_iris_write_reg(struct msm_fb_data_type *mfd,
				void __user *argp)
{
...
static int msmfb_iris_notify_scenario(struct msm_fb_data_type *mfd,
				int s)
{
	pr_info("Scenario = %s\n",
		(s == 1) ? "GRAPHICS"
...
static int msmfb_iris_notify_work_mode(struct msm_fb_data_type *mfd,
				void __user *argp)
{
...
static int msmfb_iris_frc_switch(struct msm_fb_data_type *mfd,
				void __user *argp)
{
	int ret;
	bool video_on;
...
static int mdss_mipi_dsi_command(struct mdss_panel_data *pdata, void __user *argp)
{
...
static int msmfb_iris_configure(struct msm_fb_data_type *mfd,
				void __user *argp)
{
...
static int msmfb_iris_configure_ex(struct msm_fb_data_type *mfd,
				void __user *argp)
{
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	//iris start
	case MSMFB_IRIS_TUNING:
		ret = msmfb_iris_tuning(mfd, argp);
		break;
	case MSMFB_IRIS_WRITE_REG:
		ret = msmfb_iris_write_reg(mfd, argp);
		break;
	case MSMFB_IRIS_NOTIFY_SCENARIO:
		ret = msmfb_iris_notify_scenario(mfd, *(int *)argp);
		break;
	case MSMFB_IRIS_NOTIFY_WORK_MODE:
		ret = msmfb_iris_notify_work_mode(mfd, argp);
		break;
	case MSMFB_IRIS_FRC_SWITCH:
		ret = msmfb_iris_frc_switch(mfd, argp);
		break;
	case MSMFB_MIPI_DSI_COMMAND:
		ret = mdss_mipi_dsi_command(mdp5_data->ctl->panel_data, argp);
		break;
	case MSMFB_IRIS_CONFIGURE:
		ret = msmfb_iris_configure(mfd, argp);
		break;
	case MSMFB_IRIS_CONFIGURE_EX:
		ret = msmfb_iris_configure_ex(mfd, argp);
		break;
	case MSMFB_IRIS_CONFIGURE_GET:
		ret = msmfb_iris_configure_get(mfd, argp);
		break;
	case MSMFB_IRIS_NOTIFY_VIDEO_FRAME_RATE:
		ret = iris_notify_video_frame_rate(mfd, argp);
		break;
	case MSMFB_IRIS_SET_META:
		ret = iris_set_meta(mfd, argp);
		break;
	case MSMFB_IRIS_SET_MODE:
		ret = iris_set_mode(mfd, argp);
		break;
	case MSMFB_IRIS_GET_MODE:
		ret = iris_get_mode(mfd, argp);
		break;
	case MSMFB_IRIS_FRC_PATH_UPDATE:
		ret = iris_frc_path_update(mfd, argp);
		break;
	case MSMFB_IRIS_SET_ROTATION:
		ret = iris_set_rotation(mfd, argp);
		break;
#if defined(CONFIG_IRIS2_DRC_SUPPORT)
	case MSMFB_IRIS_GET_FRCTIMING:
		ret = iris_get_frc_timing(mfd, argp);
	case MSMFB_IRIS_SET_DRC_SIZE:
		ret = iris_set_drc_size(mfd, argp);
#endif
	//iris end
#endif
	default:
...
int mdss_mdp_overlay_init(struct msm_fb_data_type *mfd)
{
	...
	if (iris2_get_id() == 0x1) {
		rc = sysfs_create_group(&dev->kobj,
					&vtotal_adjust_fs_attrs_group);
		if (rc)
			pr_err("Error vadj sysfs creation ret=%d\n", rc);
	}
#endif
...
#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP) || defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
	if (iris2_get_id() == 0x1)
