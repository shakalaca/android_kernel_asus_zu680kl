...
static int mmc_get_ext_csd(struct mmc_card *, u8 **);

static void _mmc_asusevent_log(struct  mmc_host *host)
{
	char* vendor;
	char* emmc_version;

	switch (host->card->cid.manfid){
		case 0x2  : vendor = "SANDISK"; break;
	...
	printk("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG,fw_version=0x%x, lifeA=0x0%x,lifeB=0x%x,pre_eol=0x%x\n",
		mmc_hostname(host), vendor, emmc_version, host->card->mmc_total_size, host->card->cid.prv,
		host->card->ext_csd.raw_device_life_time_A, host->card->ext_csd.raw_device_life_time_B,
		host->card->ext_csd.raw_pre_eol_info);

	ASUSEvtlog("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG,fw_version=0x%x, lifeA=0x0%x,lifeB=0x%x,pre_eol=0x%x\n",
		mmc_hostname(host), vendor, emmc_version, host->card->mmc_total_size, host->card->cid.prv,
		host->card->ext_csd.raw_device_life_time_A, host->card->ext_csd.raw_device_life_time_B,
		host->card->ext_csd.raw_pre_eol_info);
...
static void mmc_asusevent_log(struct mmc_host *host, u8 *asus_ext_csd)
{
	if (asus_ext_csd == NULL) {
		pr_err("%s [EMMC_STATUS] mmc_asusevent_log NULL ext_csd\n", mmc_hostname(host));
	} else {
		host->card->ext_csd.raw_device_life_time_A = asus_ext_csd[268];
		host->card->ext_csd.raw_device_life_time_B = asus_ext_csd[269];
	}

	if(pre_life_time_A != host->card->ext_csd.raw_device_life_time_A ||
		pre_life_time_B != host->card->ext_csd.raw_device_life_time_B)
		_mmc_asusevent_log(host);

	pre_life_time_A = host->card->ext_csd.raw_device_life_time_A;
	pre_life_time_B = host->card->ext_csd.raw_device_life_time_B;
...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	card->ext_csd.raw_pre_eol_info = ext_csd[267];
...
static int mmc_compare_ext_csds(struct mmc_card *card, unsigned bus_width)
{
	...
	mmc_asusevent_log(card->host, bw_ext_csd);
