...
void mmc_power_up(struct mmc_host *host)
{
	...
	if(!strcmp("mmc1", mmc_hostname(host))&& gpio_is_valid(sdhci->sd_pwr_gpio)) {
		pr_debug("%s: sd power on!\n", mmc_hostname(host));
		gpio_set_value(sdhci->sd_pwr_gpio, 1);
	}
	/*
...
void mmc_power_off(struct mmc_host *host)
{
	...
	/* disable sd slot power */
	if(!strcmp("mmc1", mmc_hostname(host))&& gpio_is_valid(sdhci->sd_pwr_gpio)) {
		pr_debug("%s: sd power off!\n", mmc_hostname(host));
		gpio_set_value(sdhci->sd_pwr_gpio, 0);
	}
