...
/* gesture id of Synaptics */
#define SYNA_DCLICK_TYPE	0x03
#define SYNA_GESTURE_TYPE	0x0b
#define SYNA_GESTURE_TYPE_V	0x0a
#define SYNA_SWIPE_TYPE		0x07
#define SYNA_GESTURE_W		0x77
#define SYNA_GESTURE_S		0x73
#define SYNA_GESTURE_E		0x65
#define SYNA_GESTURE_C		0x63
#define SYNA_GESTURE_Z		0x7a
#define SYNA_GESTURE_V		0x02
/* key code for input dev */
#define KEY_GESTURE_DCLICK	356
#define KEY_GESTURE_SWIPE_UP	KEY_WAKEUP
#define KEY_GESTURE_W		249
#define KEY_GESTURE_S		250
#define KEY_GESTURE_E		251
#define KEY_GESTURE_C		252
#define KEY_GESTURE_Z		253
#define KEY_GESTURE_V		254
...
static void synaptics_rmi4_f12_gloved(struct synaptics_rmi4_data *rmi4_data,
						bool enable);
...
static ssize_t synaptics_rmi4_touch_stay_awake_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_touch_stay_awake_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static ssize_t touch_swipeup_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t touch_swipeup_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static ssize_t touch_fw_info_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t touch_gloved_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t touch_gloved_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(touch_status, S_IRUGO,
			...
			NULL),
	__ATTR(touch_stay_awake, S_IRUGO | S_IWUSR,
			synaptics_rmi4_touch_stay_awake_show,
			synaptics_rmi4_touch_stay_awake_store),
	__ATTR(touch_irq_enable, S_IRUGO | S_IWUSR,
			synaptics_rmi4_irq_enable_show,
			synaptics_rmi4_irq_enable_store),
	__ATTR(touch_dclick, S_IRUGO | S_IWUSR,
			touch_dclick_show,
			touch_dclick_store),
	__ATTR(touch_swipeup, S_IRUGO | S_IWUSR,
			touch_swipeup_show,
			touch_swipeup_store),
	__ATTR(touch_gesture, S_IRUGO | S_IWUSR,
			touch_gesture_show,
			touch_gesture_store),
	__ATTR(tp_fw_info, S_IRUGO, touch_fw_info_show, NULL),
	__ATTR(touch_gloved, S_IRUGO | S_IWUSR,
			touch_gloved_show,
			touch_gloved_store),
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&(rmi4_data->rmi4_drvdata_mutex));

	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->enable_wakeup_gesture = input;

	mutex_unlock(&(rmi4_data->rmi4_drvdata_mutex));
		...
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval;
	unsigned char build_id[BUILD_ID_SIZE] = {0};
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("[Synaptics] touch stay_awake = %d\n", rmi4_data->stay_awake);
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("[Synaptics] touch irq_enabled = %d\n", rmi4_data->irq_enabled);
	return snprintf(buf, PAGE_SIZE, "%d\n", rmi4_data->irq_enabled);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == 0) {
		if (rmi4_data->irq_enabled) {
			...
			pr_info("[Synaptics] Disable touch irq\n");
		} else
			pr_info("[Synaptics] Touch irq already disabled\n");
	} else {
		if (!rmi4_data->irq_enabled) {
			enable_irq(rmi4_data->irq);
			rmi4_data->irq_enabled = true;
			pr_info("[Synaptics] Enable touch irq\n");
		} else
			pr_info("[Synaptics] Touch irq already enabled\n");
	}
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "[Synaptics] Double click = %d\n",
					rmi4_data->zenMotion_dclick);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1')
		rmi4_data->zenMotion_dclick = true;
	else	/*disabled if input unknown*/
		rmi4_data->zenMotion_dclick = false;

	if (rmi4_data->zenMotion_dclick || rmi4_data->zenMotion_swipeup ||
		((rmi4_data->zenMotion_gesture & 0x40) &&
		 (rmi4_data->zenMotion_gesture & 0xbf))) {
	...
	pr_info("[Synaptics] %s: set dclick = %d\n",
			__func__, rmi4_data->zenMotion_dclick);
	return count;
}
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "[Synaptics] Swipe up = %d\n",
					rmi4_data->zenMotion_swipeup);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1')
		rmi4_data->zenMotion_swipeup = true;
	else	/*disabled if input unknown*/
		rmi4_data->zenMotion_swipeup = false;
	...
	pr_info("[Synaptics] %s: set swipe up = %d\n",
			__func__, rmi4_data->zenMotion_swipeup);
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
		"[Synaptics] Gesture wseczv = %d %d %d %d %d %d\n",
				(rmi4_data->zenMotion_gesture & 0x20) >> 5,
				(rmi4_data->zenMotion_gesture & 0x10) >> 4,
				(rmi4_data->zenMotion_gesture & 0x08) >> 3,
				(rmi4_data->zenMotion_gesture & 0x04) >> 2,
				(rmi4_data->zenMotion_gesture & 0x02) >> 1,
				(rmi4_data->zenMotion_gesture & 0x01));
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/*only need to read wseczv bit from input*/
	for (i = 0; i <= ASUS_GESTURE_COUNT; i++){
		if (buf[i] == '1') {
			gesture_sel =
				gesture_sel | (0x1 << (ASUS_GESTURE_COUNT - i));
		} else { /*disabled if input unknown*/
			gesture_sel =
				gesture_sel & ~(0x1 << (ASUS_GESTURE_COUNT - i));
		}
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: gesture_sel = %x\n", __func__, gesture_sel);

	mutex_lock(&(rmi4_data->rmi4_drvdata_mutex));

	rmi4_data->zenMotion_gesture = gesture_sel;
	...
	pr_info("[Synaptics] set gesture m-wseczv = %d-%d %d %d %d %d %d\n",
				(rmi4_data->zenMotion_gesture & 0x40) >> 6,
				(rmi4_data->zenMotion_gesture & 0x20) >> 5,
				(rmi4_data->zenMotion_gesture & 0x10) >> 4,
				(rmi4_data->zenMotion_gesture & 0x08) >> 3,
				(rmi4_data->zenMotion_gesture & 0x04) >> 2,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (fwu)
		pr_info("[Synaptics] Bootloader version = %u\n", fwu->bl_version);
	else {
		pr_info("[Synaptics] fwu handle not exist\n");
		return snprintf(buf, PAGE_SIZE, "Read BL version failed\n");
	}
	...
	if (retval < 0) {
		pr_info("[Synaptics] read firmware ID failed\n");
		return snprintf(buf, PAGE_SIZE, "Read FW ID failed\n");
	} else {
		firmware_id = (unsigned int)build_id[0] +
			(unsigned int)build_id[1] * 0x100 +
			(unsigned int)build_id[2] * 0x10000;
	...
	if (fwu) {
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					config_id_size);
		if (retval < 0) {
			pr_info("[Synaptics] read config ID failed\n");
			return snprintf(buf, PAGE_SIZE, "Read CFG ID failed\n");
		} else {
			pr_info("[Synaptics] config ID = %02x%02x%02x%02x\n",
	...
	return snprintf(buf, PAGE_SIZE,
		"BL-FW-CFG = %d-%u-%02x%02x%02x%02x\n", fwu->bl_version, firmware_id,
							config_id[0], config_id[1],
							config_id[2], config_id[3]);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	offset = extra_data->ctrl26_offset;

	retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + offset,
			&ctrl26_control,
			sizeof(ctrl26_control));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f12 ctrl26\n",
				__func__);
	}

	pr_info("[Synaptics] glove enabled = %d, ctrl26_gloved = 0x%x\n",
			rmi4_data->enable_gloved, (ctrl26_control & 0x01));

	return scnprintf(buf, PAGE_SIZE,
		"[Synaptics] glove enabled = %d, ctrl26_gloved = 0x%x\n",
			rmi4_data->enable_gloved, (ctrl26_control & 0x01));
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rmi4_data->enable_gloved = input;

	if (!rmi4_data->auto_update_finish) {
		pr_info("[Synaptics] %s:startup fw updating, action abort\n",
								__func__);
	} else
		synaptics_rmi4_f12_gloved(rmi4_data, rmi4_data->enable_gloved);

	mutex_unlock(&(rmi4_data->rmi4_drvdata_mutex));

	pr_info("[Synaptics] touch glove enabled = %d\n", input);
	return count;
}

static void synaptics_gesture_report(struct synaptics_rmi4_data *rmi4_data,
						unsigned char gesture_type,
						unsigned char gesture_id,
						unsigned char swipe_x_dist)
{
	if (gesture_type == SYNA_DCLICK_TYPE) {
		if (rmi4_data->zenMotion_dclick) {
			pr_info("[Synaptics] double click\n");
			input_report_key(rmi4_data->input_dev,
						KEY_GESTURE_DCLICK, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev,
						KEY_GESTURE_DCLICK, 0);
			input_sync(rmi4_data->input_dev);
		}
	} else if (gesture_type == SYNA_GESTURE_TYPE ||
				gesture_type == SYNA_GESTURE_TYPE_V) {
		switch (gesture_id) {
		case SYNA_GESTURE_W:
			if (rmi4_data->zenMotion_gesture & 0x20) {
				pr_info("[Synaptics] gesture_id = 0x%x (W)\n",
								gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_W, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		case SYNA_GESTURE_S:
			if (rmi4_data->zenMotion_gesture & 0x10) {
				pr_info("[Synaptics] gesture_id = 0x%x (S)\n",
								gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_S, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		case SYNA_GESTURE_E:
			if (rmi4_data->zenMotion_gesture & 0x08) {
				pr_info("[Synaptics] gesture_id = 0x%x (E)\n",
								gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_E, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		case SYNA_GESTURE_C:
			if (rmi4_data->zenMotion_gesture & 0x04) {
				pr_info("[Synaptics] gesture_id = 0x%x (C)\n",
								gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_C, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		case SYNA_GESTURE_Z:
			if (rmi4_data->zenMotion_gesture & 0x02) {
				pr_info("[Synaptics] gesture_id = 0x%x (Z)\n",
								gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_Z, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		case SYNA_GESTURE_V:
			if (rmi4_data->zenMotion_gesture & 0x01) {
				pr_info("[Synaptics] gesture_id = 0x%x (V)\n",
									gesture_id);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_V, 1);
				input_sync(rmi4_data->input_dev);
			...
			break;
		default:
			pr_info("[Synaptics] unknown gesture id\n");
			break;
		}
	} else if (gesture_type == SYNA_SWIPE_TYPE) {
		if (rmi4_data->zenMotion_swipeup) {
			dev_info(rmi4_data->pdev->dev.parent,
				"x_dist = %d, y_dist = %d\n",
				(int)swipe_x_dist, (int)gesture_id);

			/*restrict swipe direction(up) and angle tolerance*/
			if (gesture_id >= 128 &&
				(swipe_x_dist <= 50 || swipe_x_dist >= 205)) {
					pr_info("[Synaptics] SWIPE UP\n");

				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_SWIPE_UP, 1);
				input_sync(rmi4_data->input_dev);
				input_report_key(rmi4_data->input_dev,
							KEY_GESTURE_SWIPE_UP, 0);
				input_sync(rmi4_data->input_dev);
			}

		}
	} else
		pr_info("[Synaptics] unknown gesture type\n");
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		dev_dbg(rmi4_data->pdev->dev.parent,
				"%s: gesture_det = 0x%x, %x, %x, %x, %x\n",
				__func__,
				rmi4_data->gesture_detection[0],
				rmi4_data->gesture_detection[1],
				rmi4_data->gesture_detection[2],
				rmi4_data->gesture_detection[3],
				rmi4_data->gesture_detection[4]);

		gesture_type = rmi4_data->gesture_detection[0];

		if (gesture_type && gesture_type != F12_UDG_DETECT) {
			synaptics_gesture_report(rmi4_data, gesture_type,
					rmi4_data->gesture_detection[2],
					rmi4_data->gesture_detection[1]);
		}
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			dev_dbg(rmi4_data->pdev->dev.parent,
					"%s: Finger %d: status = 0x%02x, x = %d, y = %d, wx = %d, wy = %d, z = %d\n",
					__func__, finger,
					finger_status,
					x, y, wx, wy, z);
...
flash_prog_mode:
	...
	dev_info(rmi4_data->pdev->dev.parent, "FW build ID = %u\n",
						rmi4_data->firmware_id);
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		retval = gpio_request(gpio, "syna_dsx_gpio");
		if (retval) {
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/*JB comment, the vir_button_map is NULL*/
	/*
	...
	*/
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(KEY_GESTURE_DCLICK, rmi4_data->input_dev->keybit);
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	printk("[Synaptics] step 1. irq_gpio=%d, p_gpio=%d, r_gpio=%d\n",
		gpio_get_value(bdata->irq_gpio),
		gpio_get_value(bdata->power_gpio),
		gpio_get_value(bdata->reset_gpio));
	...
	printk("[Synaptics] step2. irq_gpio=%d, p_gpio=%d, r_gpio=%d\n",
		gpio_get_value(bdata->irq_gpio),
		gpio_get_value(bdata->power_gpio),
		gpio_get_value(bdata->reset_gpio));
	...
	if (bdata->power_gpio >= 0) {
		...
		printk("[Synaptics] step3. irq_gpio=%d, p_gpio=%d, r_gpio=%d\n",
			gpio_get_value(bdata->irq_gpio),
			gpio_get_value(bdata->power_gpio),
			gpio_get_value(bdata->reset_gpio));
	}
	...
	if (bdata->reset_gpio >= 0) {
		...
		printk("[Synaptics] step4. irq_gpio=%d, p_gpio=%d, r_gpio=%d\n",
			gpio_get_value(bdata->irq_gpio),
			gpio_get_value(bdata->power_gpio),
			gpio_get_value(bdata->reset_gpio));
	...
	select_irq_pin_state(rmi4_data->pdev->dev.parent, true);

	printk("[Synaptics] step5. irq_gpio=%d, p_gpio=%d, r_gpio=%d\n",
		gpio_get_value(bdata->irq_gpio),
		gpio_get_value(bdata->power_gpio),
		gpio_get_value(bdata->reset_gpio));
...
static int synaptics_rmi4_sw_reset(struct synaptics_rmi4_data *rmi4_data)
{
	...
	mdelay(rmi4_data->hw_if->board_data->reset_delay_ms);
...
static void synaptics_rmi4_rebuild_work(struct work_struct *work)
{
	...
	dev_info(rmi4_data->pdev->dev.parent, "REBUILD deivce\n");
		...
		bool rebuild)
{
	...
	dev_info(rmi4_data->pdev->dev.parent, "RESET deivce\n");
...
static ssize_t synaptics_touch_switch_name(struct switch_dev *sdev, char *buf)
{
	int count = 0;
	int retval;
	struct synaptics_rmi4_data *rmi4_data =
			container_of(sdev, struct synaptics_rmi4_data,
			touch_sdev);
	unsigned char build_id[BUILD_ID_SIZE] = {0};
	unsigned int firmware_id = 0;
	unsigned char config_id[32] = {0};
	...
	if (retval < 0) {
		...
		firmware_id = (unsigned int)build_id[0] +
			...
			(unsigned int)build_id[2] * 0x10000;
		pr_info("[Synaptics] firmware ID = %u\n", firmware_id);
		count += sprintf(buf + count, "%u", firmware_id);
	}
	...
	if (fwu) {
		...
		if (retval < 0) {
			...
			pr_info("[Synaptics] config ID = %02x%02x%02x%02x\n",
							config_id[0], config_id[1],
							config_id[2], config_id[3]);
			count += sprintf(buf + count, "-%02x%02x%02x%02x\n",
							config_id[0], config_id[1],
							config_id[2], config_id[3]);
		}
	} else {
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->zenMotion_dclick = false;
	rmi4_data->zenMotion_gesture = 0;
	rmi4_data->zenMotion_swipeup = false;
	rmi4_data->enable_wakeup_gesture = false;

	rmi4_data->enable_gloved = false;
	rmi4_data->auto_update_finish = false;
	...
	mutex_init(&(rmi4_data->rmi4_drvdata_mutex));

	printk("[Synaptics] data_info:\n");
	printk("[Synaptics] i2c_addr=%x, power_gpio=%d, reset_gpio=%d, irq_gpio=%d\n",
			rmi4_data->hw_if->board_data->i2c_addr,
			rmi4_data->hw_if->board_data->power_gpio,
			rmi4_data->hw_if->board_data->reset_gpio,
			rmi4_data->hw_if->board_data->irq_gpio);
	...
	*/
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			dev_err(&pdev->dev,
			"%s: Failed to create platform sysfs attributes\n",
								__func__);
			goto err_platform_sysfs;
		}
	}
	...
	rmi4_data->touch_sdev.print_name = synaptics_touch_switch_name;
	if(switch_dev_register(&rmi4_data->touch_sdev) < 0)
		dev_info(&pdev->dev, "switch_dev_register for touch failed!\n");

	printk("[Synaptics] %s:%d done\n", __func__, __LINE__);

	return retval;

err_platform_sysfs:
	for (attr_count--; attr_count >= 0; attr_count--)
		sysfs_remove_file(&pdev->dev.kobj, &attrs[attr_count].attr);
...
void synaptics_rmi4_shutdown(struct platform_device *pdev)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(pdev);
	int retval;

	pr_info("[Synaptics] shutdown, disable irq\n");

	retval = synaptics_rmi4_irq_enable(rmi4_data, false, false);
	if (retval) {
		pr_err("%s: Failed to disable and free irq\n",
					__func__);
	}
}
		...
		bool enable)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to enable has trans wakeup\n",
				__func__);
		return;
	}
		...
		bool enable)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: ctrl26 offset = 0x%x, addr = 0x%x, value = 0x%x\n",
			__func__, offset,
			fhandler->full_addr.ctrl_base + offset,
			ctrl26_control);
	if (enable)
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: ctrl26 set value = 0x%x\n",
					__func__, ctrl26_control);
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to change gloved mode\n",
				__func__);
		return;
	}

	pr_info("[Synaptics] touch glove %s\n", enable? "enabled" : "disabled");
	return;
}
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	pr_info("[Synaptics] Enter %s:%d\n", __func__, __LINE__);

	if (!rmi4_data->auto_update_finish) {
		pr_info("[Synaptics] startup fw updating, suspend abort\n");
		return 0;
	}

	if (rmi4_data->stay_awake) {
		pr_info("[Synaptics] touch stay awake\n");
		return 0;
	}
	...
	if (rmi4_data->enable_wakeup_gesture) {
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		enable_irq_wake(rmi4_data->irq);
		pr_info("[Synaptics] Enter gesture wakeup mode\n");
		goto exit;
	}
	...
	if (!rmi4_data->suspend) {
		...
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);
		pr_info("[Synaptics] Enter deep sleep mode\n");
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (!rmi4_data->auto_update_finish) {
		pr_info("[Synaptics] startup fw updating, resume abort\n");
		return 0;
	}
	...
	if (rmi4_data->enable_wakeup_gesture) {
		disable_irq_wake(rmi4_data->irq);
		pr_info("[Synaptics] Leave gesture wakeup mode\n");
	} else {
		rmi4_data->current_page = MASK_8BIT;
		synaptics_rmi4_irq_enable(rmi4_data, true, false);
		rmi4_data->sensor_sleep = false;
		pr_info("[Synaptics] Leave deep sleep mode\n");
	}
	...
	if (rmi4_data->enable_gloved)
		synaptics_rmi4_f12_gloved(rmi4_data, true);

	rmi4_data->suspend = false;

	printk("[Synaptics] resume done\n");
	return 0;
}
