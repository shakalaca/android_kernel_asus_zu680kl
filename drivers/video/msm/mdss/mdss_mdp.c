...
int msm_splash_handoff_done = 0;
EXPORT_SYMBOL(msm_splash_handoff_done);
...
void mdss_mdp_footswitch_ctrl_splash(int on)
{
	...
	if (mdata != NULL) {
		if (on) {
			...
			msm_splash_handoff_done = 1;
			pr_info("%s: splash handoff is done\n", __func__);
		}
	} else {
...
extern int Tcon_frame_rate;
static struct class *Tcon_IC_class;
static ssize_t Tcon_show_framerate(struct class *cls,
...
static struct class_attribute Tcon_IC_attributes =
	__ATTR(framerate, 0644, Tcon_show_framerate, NULL);
static int mdss_mdp_probe(struct platform_device *pdev)
{
	...
	Tcon_IC_class = class_create(THIS_MODULE, "Tcon_IC");
	if (IS_ERR(Tcon_IC_class)) {
		printk(KERN_WARNING "Unable to create usb_camera class; errno = %ld\n",
			PTR_ERR(Tcon_IC_class));
			return PTR_ERR(Tcon_IC_class);
	}
	err = class_create_file(Tcon_IC_class, &Tcon_IC_attributes);
	if (err)
		class_destroy(Tcon_IC_class);
...
int shutdown_flag = 0;
int IOVCC_gpio= 103;  //display 1V8 power
void mdss_mdp_shutdown(struct platform_device *pdev)
{
	struct mdss_data_type *mdata = platform_get_drvdata(pdev);
	struct mdss_panel_data *pdata= mdata->ctl_off->panel_data;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	int ret = 0;
	printk("Display check mdss_mdp_shutdown \n");
	shutdown_flag = 1;
	//disable VSN
	//disable VSP
	...
	if (ret)
		pr_err("%s: failed to disable vregs for VSN and VSP\n",__func__);

	mdelay(5);

	if (gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		printk("Display Check IOVCC \n");
		gpio_set_value(ctrl_pdata->disp_en_gpio, 0);
	}
	if (gpio_is_valid(ctrl_pdata->rst_gpio)) {
		printk("Display Check reset \n");
		gpio_set_value(ctrl_pdata->rst_gpio, 0);
	}
	if (gpio_is_valid(ctrl_pdata->tp_rst_gpio)) {
		printk("Display Check TP reset \n");
		gpio_set_value(ctrl_pdata->tp_rst_gpio, 0);
	}
	if (gpio_is_valid(ctrl_pdata->tp_irq_gpio)) {
		printk("Display Check TP irq \n");
		gpio_direction_output(ctrl_pdata->tp_irq_gpio, 0);
	}

	dev_dbg(&pdev->dev, "display shutdown\n");
}
#endif
