						...
						struct sdhci_host *host)
{
	...
	host->sd_pwr_gpio = of_get_named_gpio(np, "sd-pwr", 0);
	if (!gpio_is_valid(host->sd_pwr_gpio)) {
                dev_err(dev, "invalid GPIO %d\n", host->sd_pwr_gpio);
        } else {
		if (gpio_request(host->sd_pwr_gpio, "sd-pwr")) {
	                dev_err(dev, "failed to request GPIO %d\n", host->sd_pwr_gpio);
	                goto out;
	        }
		if (gpio_direction_output(host->sd_pwr_gpio, 0)) {
	                dev_err(dev, "failed to set sd-pwr: %d output low\n", host->sd_pwr_gpio);
	                goto out;
	        }
	}
