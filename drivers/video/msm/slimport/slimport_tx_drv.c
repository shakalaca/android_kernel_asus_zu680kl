...
uint slimport_chip_id;
EXPORT_SYMBOL(slimport_chip_id);
...
void SP_TX_Initialization(struct VideoFormat* pInputFormat)
{
	...
	*/
	...
	//400mv (actual 200mv)
	//Swing
	sp_write_reg(MIPI_RX_PORT1_ADDR, 0x30, 0x18);// ASUS tune DP CTS 20160523
	...
	sp_write_reg(MIPI_RX_PORT1_ADDR, 0x34, 0x33);// ASUS tune DP CTS 20160523
	sp_write_reg(MIPI_RX_PORT1_ADDR, 0x38, 0x3c);//3.5db
	//emp
	//sp_write_reg(MIPI_RX_PORT1_ADDR, 0x44, 0x00);//0db
	sp_write_reg(MIPI_RX_PORT1_ADDR, 0x48, 0x12);// ASUS tune DP CTS 20160523
...
BYTE  SP_TX_Config_Video_MIPI (void)
{
	...
	if(!bMIPI_Configured) {
		...
		sp_write_reg(MIPI_RX_PORT1_ADDR, 0x1b, 0x64); // ASUS tune DP CTS 20160523
...
BYTE SP_TX_Chip_Located(void)
{
	...
	slimport_chip_id = c1 | (c2 << 8);	/* slimport_chip_id should be 0x7805 */
