		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (enable_brightness_debug==1) {
		printk("Display check led_brightness_store value=%d \n",(int)state);
	}
...
static ssize_t led_brightness_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = -EINVAL;
	long debug = 0;
	...
	if (debug) {
		enable_brightness_debug = 1;
	} 
	else
		enable_brightness_debug = 0;
	ret = size;
	return ret;
}

static ssize_t led_brightness_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int rc = -ENXIO;
	rc = sprintf(buf, "%d\n",enable_brightness_debug);
	return rc;
}
