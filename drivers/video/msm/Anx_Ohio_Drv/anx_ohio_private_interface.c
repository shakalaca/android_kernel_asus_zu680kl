...
void handle_intr_vector(void)
{
	...
	if (intr_vector) {
		...
		vbus = OhioReadReg(OHIO_SYSTEM_STSTUS) & VBUS_STATUS;
		if (old_vbus != vbus) {
			old_vbus = vbus;
			pd_vbus_control_default_func(vbus);
...
u8 fit_rdo_from_source_caps(u8 obj_cnt, u8 *buf, enum vbus_jump mode)
{
	...
	for (i = 0; i < obj_cnt; i++) {
		...
		pdo_cal[i].watt = (pdo_max_tmp * max_request_ma >= MAX_POWER) ? MAX_POWER: pdo_max_tmp * max_request_ma;
		pdo_cal[i].index = i;
...
void fit_rdo_value(void){
	...
	if ((p_list[mode].pdo_h & (3 << 14)) != (PDO_TYPE_BATTERY >> 16)) {
		...
		pdo_max =
		    RDO_FIXED(p_list[mode].index + 1, MAX_REQUEST_CURRENT,
...
u8 recv_pd_source_caps_default_callback(void *para, u8 para_len)
{
	...
	if (para_len % 4 != 0)
		return ret;
#ifdef PD_CHARGING_DRIVER_SUPPORT
	fit_rdo_from_source_caps(para_len / 4, para, VBUS_5V);
	fit_rdo_from_source_caps(para_len / 4, para, VBUS_9V);
	fit_rdo_value();
...
void pd_cc_status_default_func(u8 cc_status)
{
	...
	u8 cc_tmp = 0;
	if (cc_status & 0xc0)
		cc_tmp = (cc_status & 0xf0) >> 4;
	else if (cc_status & 0x0c)
		cc_tmp = cc_status & 0x0f;

	if(cc_tmp == 0x0c)
		chg_type = TYPEC_3A_CHARGER;
	else if (cc_tmp == 0x08)
		chg_type = TYPEC_1_5A_CHARGER;
	else if (cc_tmp == 0x04)
		chg_type = TYPEC_UNKNOWN_CHARGER;
	if(cc_tmp)
		asus_typec_DFP_type(chg_type, 0, 0);
#endif
	/* cc status */
	#ifdef OHIO_DEBUG
	pr_info("cc status %x\n", cc_status);
