...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
int mdss_mdp_video_timegen_vtotal_adjust(struct mdss_overlay_private *mdp5_data,
					 int vadj)
{
	struct mdss_mdp_ctl *ctl;
	...
	pr_info("vadj %d\n", vadj);
	if ((int)pinfo->lcdc.v_front_porch < -vadj) {
		pr_warn("vadj %d > v_front_porch %u\n", vadj, pinfo->lcdc.v_front_porch);
		return -EINVAL;
	}
	...
	hsync_period = mdss_panel_get_htotal(&pdata->panel_info, false);
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	pr_debug("MDSS_MDP_REG_INTF_VSYNC_PERIOD_F0 = %08x MDSS_MDP_REG_INTF_DISPLAY_V_START_F0 = %08x\n",
		mdp_video_read(ctx, MDSS_MDP_REG_INTF_VSYNC_PERIOD_F0),
		mdp_video_read(ctx, MDSS_MDP_REG_INTF_DISPLAY_V_START_F0));
#endif
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
void mdss_mdp_video_timegen_ctl(struct mdss_mdp_ctl *ctl, int enable)
{
	struct mdss_mdp_video_ctx *ctx = ctl->priv_data;

	WARN(!enable && ctx->wait_pending, "WAIT_PENDING\n");
	writel_relaxed(enable, ctx->base + MDSS_MDP_REG_INTF_TIMING_ENGINE_EN);
}
#endif
