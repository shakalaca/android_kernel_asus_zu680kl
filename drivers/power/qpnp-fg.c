...
static int asus_fg_probe_done = 0;
module_param_named(
	gaugeIC_status, asus_fg_probe_done, int, S_IRUGO
);
...
static int get_prop_capacity(struct fg_chip *chip)
{
	...
	if (chip->charge_full) {
		pr_info("charge full flag = true, reporting 100%%\n");
...
static int asus_temp_shift_interpolation(int target,
		int a, int b, int fa, int fb)
{
	int temp = (target-a);
	return (int)(fa + ((fb-fa)*temp)/(b-a));
}
...
static void update_temp_data(struct work_struct *work)
{
	...
	int unused, fg_current, shift_value = 0;
	struct fg_chip *chip = container_of(work,
				struct fg_chip,
...
wait:
	...
	if (fg_current < 0) {
		...
		if (fg_current >= 2000 && fg_current < 3000) {
			shift_value = asus_temp_shift_interpolation(fg_current,
					2000, 3000, 42, 95);
			unused = 1;
		} else if (fg_current >= 3000 && fg_current < 4000) {
			shift_value = asus_temp_shift_interpolation(fg_current,
					3000, 4000, 95, 175);
			unused = 1;
		} else if (fg_current >= 4000 && fg_current < 6000) {
			shift_value = asus_temp_shift_interpolation(fg_current,
					4000, 6000, 175, 400);
			unused = 1;
		}
	...
	if (unused) {
		pr_debug("***** shift temperature %d /10 C due to large current: %d mA\n",
			shift_value, fg_current);
		fg_data[0].value = fg_data[0].value - shift_value;
	}
	if (fg_debug_mask & FG_MEM_DEBUG_READS)
...
static enum power_supply_property fg_power_props[] = {
	...
	POWER_SUPPLY_PROP_CSC_BATTERY_SOH,
};
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW_RAW:
		...
		break;
	case POWER_SUPPLY_PROP_CSC_BATTERY_SOH:
		val->intval = 1;
		break;
	default:
		return -EINVAL;
...
static void fg_cap_learning_stop(struct fg_chip *chip)
{
	...
	pr_info("stop learning...clear learning_data.active and wake lock\n");
...
static void fg_cap_learning_work(struct work_struct *work)
{
	...
	if (!chip->learning_data.active) {
		pr_info("learning_data.active = false... clear learning wake lock\n");
		fg_relax(&chip->capacity_learning_wakeup_source);
		goto fail;
	}
...
static int fg_cap_learning_check(struct fg_chip *chip)
{
	...
	if (chip->status == POWER_SUPPLY_STATUS_CHARGING
		...
		if (chip->wa_flag & USE_CC_SOC_REG) {
			...
			pr_info("set learning_data.active = true\n");
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		pr_info("battery status update: %d -> %d\n", chip->prev_status, chip->status);
...
static irqreturn_t fg_soc_irq_handler(int irq, void *_chip)
{
	...
	if (chip->wa_flag & USE_CC_SOC_REG
			&& chip->learning_data.active) {
		pr_info("hold learning wake lock here\n");
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	asus_fg_probe_done = 1;
	asus_batt_dev.name = "battery";
	asus_batt_dev.print_name = asus_batt_switch_name;
	switch_dev_register(&asus_batt_dev);
	return rc;
