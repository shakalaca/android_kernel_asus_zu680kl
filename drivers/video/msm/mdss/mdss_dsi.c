...
static void mdss_dsi_dba_work(struct work_struct *work)
{
	...
	utils_init_data.chip_name = "anx7805";
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
		...
		if (iris2_get_id() == 0x1) {
			if (boot_cnt == 0) {
				iris_fw_download_cont_splash(pdata, 0);
				boot_cnt = 1;
			}
		}
#endif
	...
	case MDSS_EVENT_FB_REGISTERED:
		...
		if (IS_ENABLED(CONFIG_MSM_DBA) &&
			...
			pr_info("%s: MDSS_EVENT_FB_REGISTERED event dba_work !!\n", __func__);
				queue_delayed_work(ctrl_pdata->workq,
					&ctrl_pdata->dba_work, HZ);
		}
		break;
		...
		struct platform_device *pdev)
{
	...
	if (iris2_get_id() == 0x1) {
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris", 0);
	} else
#endif
	dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->tp_rst_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-TPrst-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->tp_rst_gpio))
		pr_err("%s:%d, touch reset gpio not specified\n",
						__func__, __LINE__);
	ctrl_pdata->tp_irq_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-TPirq-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->tp_irq_gpio))
		pr_err("%s:%d, touch irq gpio not specified\n",
						__func__, __LINE__);
	...
	struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	rc = of_property_read_u32(ctrl_pdev->dev.of_node, "cell-index", &index);
	if (rc) {
		dev_err(&ctrl_pdev->dev, "%s: Cell-index not specified, rc=%d\n",
			__func__, rc);
		return rc;
	}
...
#if defined(CONFIG_PIXELWORKS_IRIS_LIGHT_UP) || defined(CONFIG_PIXELWORKS_IRIS_SUPPORT)
	if (iris2_get_id() == 0x1)
		mdss_dsi_iris_init_pdata(&(ctrl_pdata->panel_data));
#endif
