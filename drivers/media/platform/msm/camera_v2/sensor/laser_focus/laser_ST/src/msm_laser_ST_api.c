...
int Laser_whether_apply_calib(bool apply){
	...
	rc = VL53L0_GetDeviceInfo(ST_data_t, &DeviceInfo);
	if(rc == VL53L0_ERROR_NONE)
	{
		pr_debug("VL53L0_GetDeviceInfo:\n");
		pr_debug("Device Name : %s\n", DeviceInfo.Name);
		pr_debug("Device Type : %s\n", DeviceInfo.Type);
		pr_debug("Device ID : %s\n", DeviceInfo.ProductId);
		pr_info("VL5310 ProductRevisionMajor : %d, ProductRevisionMinor: %d\n",
			DeviceInfo.ProductRevisionMajor, DeviceInfo.ProductRevisionMinor);

		if ((DeviceInfo.ProductRevisionMajor != 1) ||
			((DeviceInfo.ProductRevisionMinor != 1) && (DeviceInfo.ProductRevisionMinor != 0))) {
			pr_err("Error expected cut 1.1\1.0 but found cut %d.%d\n",
				DeviceInfo.ProductRevisionMajor, DeviceInfo.ProductRevisionMinor);
		}
	}
	...
	if(timedMeasure){
		...
		VL53L0_SetInterMeasurementPeriodMilliSeconds(ST_data_t,10);
		ClosingLaser = false;
		printk("%s start contunous timed measure\n",__func__);
...
void ST_keep_measure_work(struct work_struct *work){
	...
	status = VL53L0_StartMeasurement(ST_data_t);
	printk("%s: start (%d)",__func__,status);

	do{	
		status = VL53L0_GetMeasurementDataReady(ST_data_t, &dataReady);
		if(status ==0) {
			status = VL53L0_GetRangingMeasurementData(ST_data_t, &pRangeData);
			VL53L0_ClearInterruptMask(ST_data_t, VL53L0_REG_SYSTEM_INTERRUPT_GPIO_NEW_SAMPLE_READY);
		}
...
static ssize_t ST_device_calibration_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	case MSM_LASER_FOCUS_APPLY_OFFSET_CALIBRATION:
		...
		if(ret != 0)
			pr_err("ReferenceCalibration fail %d\n", ret);
		else
...
#define VL6180_IOC_MAGIC                      ('W')
#define ASUS_LASER_SENSOR_MEASURE     _IOR(VL6180_IOC_MAGIC  , 0, unsigned int[ASUS_LASER_DATA_SIZE])
#define ASUS_LASER_SENSOR_GET_NAME	_IOR(VL6180_IOC_MAGIC  , 4, char[ASUS_LASER_NAME_SIZE])

int VL6180_get_measure(int* distance){
	int RawRange = 0, rc=0;
	VL53L0_RangingMeasurementData_t RangeData;
	//pr_err("VL6180_get_measure +\n");
	mutex_lock(&vl6180x_mutex);
	...
	* distance = RawRange;
	mutex_unlock(&vl6180x_mutex);
	//pr_err("VL6180_get_measure -, RawRange=%d, DMax=%d, STerrorStatus=%d\n", RawRange, DMax, STerrorStatus);

	return 0;
}

static int VL6180_misc_open(struct inode *inode, struct file *file){
	int rc = 0;
	client++;
	...
	if(client == 1){
		timedMeasure = true;
		rc = Laser_whether_apply_calib(1); /* apply_calibibration */
		if (rc < 0) {
			mutex_lock(&vl6180x_mutex);
			Laser_device_off();
			mutex_unlock(&vl6180x_mutex);
			Laser_whether_apply_calib(0); /* no apply_calibibration */
		}
	}
...
static int VL6180_misc_release(struct inode *inode, struct file *file)
{
	client--;
...
static long VL6180_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case ASUS_LASER_SENSOR_MEASURE:
			VL6180_get_measure(&distance);
			//__put_user(dist, (int* __user*)arg);
			dist[0] = distance;
			dist[1] = STerrorStatus;
			dist[2] = DMax;
	...
	if(cnt>=50){
		...
		LOG_Handler(LOG_DBG,"read data stime duration: %ld\t [%d,%d]\n",(((now.tv_sec*1000000)+now.tv_usec)-((start.tv_sec*1000000)+start.tv_usec))/1000,distance,STerrorStatus);
		cnt=0;
	}
...
static struct file_operations VL6180_fops = {
  .owner = THIS_MODULE,
  .open = VL6180_misc_open,
  .release = VL6180_misc_release,
  .unlocked_ioctl = VL6180_misc_ioctl,
  .compat_ioctl = VL6180_misc_ioctl
};

struct miscdevice VL6180_misc = {
  .minor = MISC_DYNAMIC_MINOR,
  .name = MODULE_NAME,
  .fops = &VL6180_fops
};

static int VL6180_misc_register(void)
{
	int rtn = 0;

	rtn = misc_register(&VL6180_misc);
	if (rtn < 0) {
		LOG_Handler(LOG_ERR,"Unable to register misc devices\n");
		misc_deregister(&VL6180_misc);
	}
	return rtn;
}
/* ----------------------------- */
...
static int msm_laser_ST_i2c_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	laserSTx_t->act_device_type = MSM_CAMERA_I2C_DEVICE;
	laserSTx_t->i2c_client = &msm_laser_focus_i2c_client;
	laserSTx_t->i2c_client->client = client;
	if (NULL == laserSTx_t->i2c_client) {
		pr_err("%s i2c_client NULL\n", __func__);
		return -EFAULT;
	}

	rc = get_dtsi_data(client->dev.of_node, laserSTx_t);
	if (rc < 0) {
	...
	if (laserSTx_t->sensordata->slave_info->sensor_slave_addr)
		laserSTx_t->i2c_client->client->addr =
			laserSTx_t->sensordata->slave_info->sensor_slave_addr;
	pr_info("laserSTx_t->i2c_client->client->addr=0x%x\n", laserSTx_t->i2c_client->client->addr);


	if (!laserSTx_t->i2c_client->i2c_func_tbl)
		laserSTx_t->i2c_client->i2c_func_tbl = &msm_sensor_qup_func_tbl;
...
static void __exit msm_laser_ST_i2c_remove(void)
{
...
static struct i2c_driver vl6180_i2c_driver = {
	...
	.probe  = msm_laser_ST_i2c_probe,
	.remove = __exit_p(msm_laser_ST_i2c_remove),
	.driver = {
		.name = LASER_NAME,
		.owner = THIS_MODULE,
...
static int __init Laser_init_module(void)
{
	...
	if (rc == 0) {
		...
		rc = i2c_add_driver(&vl6180_i2c_driver);
		if (!rc)
