...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
		if (iris2_get_id() == 0x1) {
			pr_warn("%s:Invalid response cmd cmd %x\n", __func__, cmd);
		} else {
...
static int mdss_dsi_wait4video_eng_busy(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	u32 time_to_end_of_vbl_ns;
	ktime_t wakeup_time;

	if (iris2_get_id() == 0x1) {
		if (!mdss_mdp_display_wakeup_time(mdata->ctl_off, &wakeup_time,
						  &time_to_end_of_vbl_ns)) {
			/* don't wait if we're more than 1ms before BLLP
			 * (enough time to send 64KB command in HS mode)
			 */
			if (time_to_end_of_vbl_ns > 1000000)
					return ret;
			time_to_end_of_vbl_ns /= 1000;
			/* wait until BLLP finished (plus some safety margin) */
			time_to_end_of_vbl_ns += 250;
			usleep(time_to_end_of_vbl_ns);
			return time_to_end_of_vbl_ns / 1000;
		}
	}
#endif
...
#ifdef CONFIG_PIXELWORKS_IRIS_SUPPORT
	if (iris2_get_id() == 0x1) {
		if (status & 0xcc000000) { /* INTERLEAVE_OP_CONTENTION */
