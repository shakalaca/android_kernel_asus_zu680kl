...
static struct switch_dev hs_status;
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (mbhc->impedance_detect && (jack_type != SND_JACK_HEADSET) &&
			...
			pr_info("zl = %d, zr = %d\n", mbhc->zl, mbhc->zr);
			i = 0;
			while ((((mbhc->zl == 0) && (mbhc->zr == 0)) ||
				((mbhc->zl == 0xffffffe) || (mbhc->zr == 0xffffffe)))
				  && i < IMPEDANCE_RETRY)  {
				msleep(100);
				mbhc->mbhc_cb->compute_impedance(mbhc,
						&mbhc->zl, &mbhc->zr);
				pr_info("re-compute round %d; zl = %d, zr = %d\n", i, mbhc->zl, mbhc->zr);
				i++;
			}
...
report:
	...
	switch_set_state(&hs_status, mbhc->current_plug);
...
{
	switch (switch_get_state(&hs_status)){
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_GND_DET_EN, mbhc->gnd_en);
		      ...
		      bool impedance_det_en)
{
	...
	const char *gnd_enable = "qcom,msm-mbhc-gnd-enable";
	...
	ret = of_property_read_u32(card->dev->of_node, gnd_enable, &gnd_en);
	if (ret) {
		dev_err(card->dev,
			"%s: missing %s in dt node\n", __func__, gnd_enable);
		goto err;
	...
	hs_status.name = "h2w";
	hs_status.print_name = headset_name_show;
	hs_status.print_state = headset_state_show;
	...
	ret = switch_dev_register(&hs_status);
	if (ret < 0)
		pr_err("%s: headset status node creation failed.\n", __func__);
