				      ...
				      enum led_brightness value)
{
	...
	if (tmp_backlight == 1 && value == 0) {
		tmp_backlight = 0;
		if (first_boot==1) {
			printk("Disable backlight at first time suspend \n");
			first_boot = 0;
			mdss_fb_set_backlight(mfd, 2);
			mdelay(5);
			mdss_fb_set_backlight(mfd, 1);
			mdelay(5);
			mdss_fb_set_backlight(mfd, 0);
		}
		printk("Display : disable backlight \n");
	} else if ((tmp_backlight == 0) && (value > 0)) {
		tmp_backlight = 1;
		printk("Display : enable backlight \n");
	}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* WAR: skip for fb2 to avoid system crash */
	if (mfd->index != 2) {
