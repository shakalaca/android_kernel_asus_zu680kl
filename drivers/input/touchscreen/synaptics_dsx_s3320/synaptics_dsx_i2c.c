...
*/
	...
	retval = select_irq_pin_state(dev, false);
	if (retval)
		printk("[Synaptics] set pinctrl state failed\n");

	return 0;
}

int select_irq_pin_state(struct device *dev, bool is_active)
{
	struct pinctrl *ts_pinctrl;
	struct pinctrl_state *set_state;
	int retval;

	printk("[Synaptics] set pinctrl state\n");
	ts_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(ts_pinctrl)) {
		printk("[Synaptics] can't get pinctrl\n");
		ts_pinctrl = NULL;
		return -1;
	}
	if (ts_pinctrl) {
		if (is_active) {
			set_state = pinctrl_lookup_state(ts_pinctrl,
							"pmx_ts_int_active");
			if (IS_ERR(set_state)) {
				printk("[Synaptics] can't get pinctrl state\n");
				return -1;
			}
		} else {
			set_state = pinctrl_lookup_state(ts_pinctrl,
		...
		retval = pinctrl_select_state(ts_pinctrl, set_state);
		if (retval) {
			printk("[Synaptics] can't set pinctrl state\n");
			return -1;
		}
	}
	printk("[Synaptics] set pinctrl state done\n");
	return 0;
}
#endif
		...
		const struct i2c_device_id *dev_id)
{
	...
	printk("[Synaptics] %s:%d client->addr=%d\n",
