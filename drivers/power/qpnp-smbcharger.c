...
struct smbchg_chip {
	...
	int asus_thermal_policy_flag;
	int asus_dual_flag;
	int asus_show_qc_flag;
	int asus_hvdcp_flag;
	int asus_hvdcp_rerun_flag;
	int asus_batt_last_state;
	int adcpwren_gpio;
	int adc_sw_en_gpio;
	u16			peek_poke_address;
	bool asus_cable_online;
	bool asus_is_first_run;
	bool asus_is_otg;
	struct delayed_work		asus_bc_1_2_det_work;
	struct delayed_work		asus_JEITA_check_work;
	struct delayed_work		asus_thermal_policy_work;
	struct delayed_work		asus_adapter_detect_work;
	struct delayed_work		asus_update_battery_work;
	struct mutex		asus_jeita_setting_lock;
	struct wake_lock	asus_jeita_setting_wake_lock;
	struct wake_lock	asus_adapter_detect_wake_lock;
	struct wake_lock	asus_update_battery_wake_lock;
...
static struct smbchg_chip *g_smbchg_chip;
static struct workqueue_struct *smbchg_wq;
static int asus_force_usb_suspend = 0;
static char* ASUS_CHG_TYPE;
extern int qpnp_pon_system_pwr_off(enum pon_power_off_type type);
#ifdef PD_CHARGING_DRIVER_SUPPORT
extern void asus_phy_chg_detect(void);
#endif
extern void asus_Dual_Disable(void);
extern void asus_Dual_Enable(int, int);
extern void asus_smb1351_initial_setting(void);
extern void asus_smb1351_boost_enable(int enable, bool high_current, bool polling);
extern void asus_smb1351_JEITA_rule(void);
extern void asus_smb1351_enable_wdt(bool enable);
extern u8 us5587_dump_value(void);
extern u16 us5587_read_pmi8952_thermal(void);
extern u16 us5587_read_smb1351_thermal(void);
#ifdef PD_CHARGING_DRIVER_SUPPORT
extern void usbpd_jump_voltage(void);
bool DFP_notify = false;
#else
void usbpd_jump_voltage(void){ return;}
#endif
extern int g_ftm_mode;
extern int g_reverse_charger_mode;
extern int g_charger_mode;
static void asus_JEITA_rule(struct smbchg_chip *chip);
int asus_otg_boost_enable(int enable, bool high_current);
static int asus_force_hvdcp_setting(struct smbchg_chip *chip, bool is_hv);
static int smbchg_battery_set_property(struct power_supply *psy,
				       enum power_supply_property prop,
				       const union power_supply_propval *val);
static void asus_auto_pulse_up_to_9V(struct smbchg_chip *chip);
static void asus_auto_pulse_back_to_5V(struct smbchg_chip *chip);
static irqreturn_t src_detect_handler(int irq, void *_chip);
static void asus_set_high_usb_chg_current(struct smbchg_chip *chip, int current_ma);

enum ASUS_THERMAL_LEVEL {
	asus_thermal_level0 = 0,
	asus_thermal_level1,
	asus_thermal_level2,
	asus_thermal_level3,
	asus_thermal_level_num,
};

enum ASUS_BATT_TEMP_STATE {
	UNKNOWN_STATE = 0,
	less_than_0,
	from_0_to_100,
	from_100_to_200,
...
static int asus_disable_thermal_policy = 0;
module_param_named(
	asus_thermal_policy_disable, asus_disable_thermal_policy, int, S_IRUGO | S_IWUSR
);
...
static int smbchg_debug_mask = PR_STATUS | PR_MISC;
module_param_named(
...
static int smbchg_charging_en(struct smbchg_chip *chip, bool en)
{
	...
	if (g_ftm_mode && asus_charger_limit_en) {
...
static int smbchg_usb_suspend(struct smbchg_chip *chip, bool suspend)
{
	...
	if (g_ftm_mode && asus_force_usb_suspend) {
		pr_info("force usb suspend by user\n");
		suspend = asus_force_usb_suspend;
		asus_Dual_Disable();
	}
							...
							int current_ma)
{
	...
	pr_info("Setting usb current max : %d mA\n", current_ma);
...
static inline void asus_dump_reg(struct smbchg_chip *chip, u16 addr,
		const char *name, struct seq_file *m)
{
	u8 reg;

	smbchg_read(chip, &reg, addr, 1);
	seq_printf(m, "%s - 0x%04x = 0x%02x\n", name, addr, reg);
}

static int asus_dump_regs(struct seq_file *m, void *data)
{
	...
	for (addr = 0xB; addr <= 0x10; addr++)
		asus_dump_reg(chip, chip->chgr_base + addr, "CHGR Status", m);
	for (addr = 0xF0; addr <= 0xFF; addr++)
		asus_dump_reg(chip, chip->chgr_base + addr, "CHGR Config", m);
	/* OTG interface peripheral */
	for (addr = 0xF0; addr <= 0xFF; addr++)
		asus_dump_reg(chip, chip->otg_base+ addr, "OTG", m);
	/* battery interface peripheral */
	asus_dump_reg(chip, chip->bat_if_base + RT_STS, "BAT_IF Status", m);
	asus_dump_reg(chip, chip->bat_if_base + CMD_CHG_REG, "BAT_IF Command", m);
	for (addr = 0xF0; addr <= 0xFF; addr++)
		asus_dump_reg(chip, chip->bat_if_base + addr, "BAT_IF Config", m);
	/* usb charge path peripheral */
	for (addr = 0x7; addr <= 0x10; addr++)
		asus_dump_reg(chip, chip->usb_chgpth_base + addr, "USB Status", m);
	asus_dump_reg(chip, chip->usb_chgpth_base + CMD_IL, "USB Command", m);
	for (addr = 0xF0; addr <= 0xFF; addr++)
		asus_dump_reg(chip, chip->usb_chgpth_base + addr, "USB Config", m);
	/* dc charge path peripheral */
	asus_dump_reg(chip, chip->dc_chgpth_base + RT_STS, "DC Status", m);
	for (addr = 0xF0; addr <= 0xFF; addr++)
		asus_dump_reg(chip, chip->dc_chgpth_base + addr, "DC Config", m);
	/* misc peripheral */
	asus_dump_reg(chip, chip->misc_base + IDEV_STS, "MISC Status", m);
...
static int asus_dump_debugfs_open(struct inode *inode, struct file *file)
{
	struct smbchg_chip *chip  = inode->i_private;

	return single_open(file, asus_dump_regs, chip);
}

static const struct file_operations asus_dump_debugfs_ops = {
	.owner		= THIS_MODULE,
	.open		= asus_dump_debugfs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int asus_debug_get_reg(void *data, u64 *val)
{
	struct smbchg_chip *chip = data;
	int rc;
	u8 temp;

	rc = smbchg_read(chip, &temp, chip->peek_poke_address, 1);
	if (rc) {
		pr_err("Couldn't read reg 0x%04x rc = %d\n",
...
static int asus_debug_set_reg(void *data, u64 val)
{
	struct smbchg_chip *chip = data;
	int rc;
	u8 temp;
	...
	temp = (u8) val;
	rc = smbchg_write(chip, &temp, chip->peek_poke_address, 1);
	if (rc) {
...
DEFINE_SIMPLE_ATTRIBUTE(poke_poke_debug_ops, asus_debug_get_reg, asus_debug_set_reg, "0x%02llx\n");

static int asus_therm_t1 = 49;
static int asus_thermal_t1_get(void *data, u64 *val)
{
	*val = asus_therm_t1;
	return 0;
}

static int asus_thermal_t1_set(void *data, u64 val)
{
	asus_therm_t1 = val;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_thermal_t1_ops, asus_thermal_t1_get, asus_thermal_t1_set, "%lld\n");

static int asus_therm_t2 = 52;
static int asus_thermal_t2_get(void *data, u64 *val)
{
	*val = asus_therm_t2;
	return 0;
}

static int asus_thermal_t2_set(void *data, u64 val)
{
	asus_therm_t2 = val;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_thermal_t2_ops, asus_thermal_t2_get, asus_thermal_t2_set, "%lld\n");

static int asus_therm_t3 = 58;
static int asus_thermal_t3_get(void *data, u64 *val)
{
	*val = asus_therm_t3;
	return 0;
}

static int asus_thermal_t3_set(void *data, u64 val)
{
	asus_therm_t3 = val;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_thermal_t3_ops, asus_thermal_t3_get, asus_thermal_t3_set, "%lld\n");

static bool asus_charger_force_2A = false;
static int asus_force_2A_state_get(void *data, u64 *val)
{
	*val = asus_charger_force_2A ? 1 : 0;
	return 0;
}

static int asus_force_2A_set(void *data, u64 val)
{
	if (g_ftm_mode)
		asus_charger_force_2A = val ? true : false;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_ftm_force_2A_ops, asus_force_2A_state_get, asus_force_2A_set, "0x%02llx\n");

static int asus_charger_limit_enable_get(void *data, u64 *val)
{
	if (!g_ftm_mode)
		*val = 0;
	else
		*val = asus_charger_limit_en ? 1 : 0;
	return 0;
}

static int asus_charger_limit_enable_set(void *data, u64 val)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	int batt_cap = get_prop_batt_capacity(chip);
	if (g_ftm_mode) {
		asus_charger_limit_en = val ? 1 : 0;
		pr_info("charger limit enable set: %d\n", asus_charger_limit_en);
		if (asus_charger_limit_en && asus_charger_limit_percentage <= batt_cap)
...
DEFINE_SIMPLE_ATTRIBUTE(asus_charger_limit_enable_ops, asus_charger_limit_enable_get, asus_charger_limit_enable_set, "%lld\n");

static int asus_charger_limit_perc_get(void *data, u64 *val)
{
	if (!g_ftm_mode)
		*val = 100;
	else
		*val = asus_charger_limit_percentage;
	return 0;
}

static int asus_charger_limit_perc_set(void *data, u64 val)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	if (g_ftm_mode) {
		asus_charger_limit_percentage =  (int)val;
		pr_info("charger limit percentage set: %d\n", asus_charger_limit_percentage);
		smbchg_charging_en(chip, true);
	}
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_charger_limit_perc_ops, asus_charger_limit_perc_get, asus_charger_limit_perc_set, "%lld\n");

static int asus_battery_current_now_get(void *data, u64 *val)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	*val = -get_prop_batt_current_now(chip)/1000;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_battery_current_now_ops, asus_battery_current_now_get, NULL, "%lld\n");

static int asus_get_CHG_TYPE_now(struct seq_file *m, void *data)
{
	seq_printf(m, "%s\n", ASUS_CHG_TYPE);
	return 0;
}

static int asus_CHG_TYPE_now_open(struct inode *inode, struct file *file)
{
	struct smbchg_chip *chip  = inode->i_private;

	return single_open(file, asus_get_CHG_TYPE_now, chip);
}

static const struct file_operations asus_CHG_TYPE_now_ops = {
	.owner		= THIS_MODULE,
	.open		= asus_CHG_TYPE_now_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int asus_force_QC30_set(void *data, u64 val)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	...
	if (val) {
		asus_force_hvdcp_setting(chip, false);
		smbchg_battery_set_property(&chip->batt_psy, POWER_SUPPLY_PROP_ALLOW_HVDCP3, &ret);
	} else {
		ret.intval = 0;
		smbchg_battery_set_property(&chip->batt_psy, POWER_SUPPLY_PROP_ALLOW_HVDCP3, &ret);
		asus_force_hvdcp_setting(chip, false);
		if (chip->asus_hvdcp_flag)
			asus_force_hvdcp_setting(chip, true);
	}
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_force_QC30_ops, NULL, asus_force_QC30_set, "%lld\n");

static int asus_ftm_force_usb_suspend_get(void *data, u64 *val)
{
	*val = asus_force_usb_suspend;
	return 0;
}

static int asus_ftm_force_usb_suspend_set(void *data, u64 val)
{
	struct smbchg_chip *chip = g_smbchg_chip;

	if (g_ftm_mode) {
		asus_force_usb_suspend = val ? 1 : 0;
		pr_info("force usb in suspend set: %d\n", asus_force_usb_suspend);
		if (asus_force_usb_suspend)
			smbchg_usb_suspend(chip, true);
		else
			queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(1000));
	}
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(asus_force_usb_suspend_ops, asus_ftm_force_usb_suspend_get, asus_ftm_force_usb_suspend_set, "%lld\n");
...
static int smbchg_otg_regulator_enable(struct regulator_dev *rdev)
{
	if (g_reverse_charger_mode)
		asus_otg_boost_enable(1, true);
	else
		asus_otg_boost_enable(1, false);
	return 0;
}
...
{
	struct smbchg_chip *chip = g_smbchg_chip;
	return chip->chg_otg_enabled ? 1: 0;
}
						...
						enum power_supply_type type)
{
	...
	if (type == POWER_SUPPLY_TYPE_USB_HVDCP) {
		pr_info("waiting to identify QC 3.0 adapter...\n");
		if (chip->asus_is_first_run) {
			if (g_charger_mode)
				queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(10000));
			else
				queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(20000));
			chip->asus_is_first_run = false;
		} else
			queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(5500));
	} else if (type == POWER_SUPPLY_TYPE_USB_HVDCP_3){
		pr_info("detect QC3.0 adapter!!!\n");
		cancel_delayed_work(&chip->asus_bc_1_2_det_work);
		queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(500));
	}
...
static int force_9v_hvdcp(struct smbchg_chip *chip)
{
	...
	if (!rc) {
		pr_info("always bypass force 9V operation\n");
		return rc;
	}
	/* Force QC2.0 */
	rc = smbchg_masked_write(chip,
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	if (is_hvdcp_present(chip)) {
		...
		pr_info("no hvdcp present, queue bc 1.2 flow check\n");
		queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(500));
	}
}
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	cancel_delayed_work(&chip->asus_adapter_detect_work);
	cancel_delayed_work(&chip->asus_bc_1_2_det_work);
	cancel_delayed_work_sync(&chip->asus_thermal_policy_work);
	cancel_delayed_work_sync(&chip->asus_JEITA_check_work);

	if (chip->asus_hvdcp_rerun_flag == 1) {
		chip->asus_hvdcp_rerun_flag = 2;
		return;
	}
	 else {
		if(wake_lock_active(&chip->asus_adapter_detect_wake_lock))
			wake_unlock(&chip->asus_adapter_detect_wake_lock);
		chip->asus_hvdcp_rerun_flag = 0;
		ASUS_CHG_TYPE = "UNDEFINED";
		chip->asus_cable_online = false;
		chip->asus_thermal_policy_flag = asus_thermal_level0;
		chip->asus_dual_flag = Dual_None;
		chip->asus_show_qc_flag = 0;
		chip->asus_hvdcp_flag = 0;
		chip->asus_is_first_run = false;
#ifdef PD_CHARGING_DRIVER_SUPPORT
		...
		chip->asus_pd_curr = 0;
#endif
		gpio_set_value(chip->adc_sw_en_gpio, 0);
		gpio_set_value(chip->adcpwren_gpio, 0);
		asus_smb1351_enable_wdt(false);
	}
...
#define MISC_WD_CFG_REG	0xF1
#define CFG_SYSMIN_REG		0xF4
#define CHGR_CFG1			0xFB
#define PCC_CFG_REG 0xF1
#define FCC_CFG_REG 0xF2
#define FV_CFG_REG 0xF4
#define TCC_CFG_REG 0xF9
#define CHGR_CFG_REG 0xFF
#define PCC_CFG_MASK 0x7
#define FCC_CFG_MASK 0x1F
#define FV_CFG_MASK 0x3F
#define TCC_CFG_MASK 0x7
#define CFG_RCHG_LVL_MASK BIT(0)
#define USBIN_AICL_RERUN_MASK BIT(5)
#define CHG_EN_POLARITY_BIT		BIT(6)
static void asus_initial_settings(struct smbchg_chip *chip)
{
	int rc;
	// SMBCHGL_CHGR_PCC_CFG = PCC_200MA, 0x10F1 = 0x02
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + PCC_CFG_REG, PCC_CFG_MASK, 0x2);
	if (rc)
		pr_err("set Pre-charge current = 200 mA failed\n");
	// SMBCHGL_CHGR_FCC_CFG = FCC_1200MA, 0x10F2= 0x0D
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + FCC_CFG_REG, FCC_CFG_MASK, 0x0D);
	if (rc)
		pr_err("set Fast-charge current = 1200 mA failed\n");
	// SMBCHGL_CHGR_FV_CFG = FV_4P38, 0x10F4 = 0x2D
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + FV_CFG_REG, FV_CFG_MASK, 0x2D);
	if (rc)
		pr_err("set Float Voltage =  4.38V failed\n");
	// SMBCHGL_CHGR_CFG.CFG_RCHG_LVL = RCHG_THRESH_200MV, 0x10FF[0] = '1'
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CFG_REG, CFG_RCHG_LVL_MASK, 0x1);
	if (rc)
		pr_err("set Recharge threshold =  200 mV failed\n");
	// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_900MA, 0x13F2[4:0] = 0x09
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base+ IL_CFG, USBIN_INPUT_MASK, 0x9);
	if (rc)
		pr_err("set USB IN input Current =  1000 mA failed\n");
	// SMBCHGL_CHGR_CFG_TCC = TERM_ANA_150MA, 0x10F9[2:0] = 0x03
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + TCC_CFG_REG, TCC_CFG_MASK, 0x3);
	if (rc)
		pr_err("set Termination Current =  150 mA failed\n");
	// TEWM_I_SRC = FUEL GAUGE_ADC, RECHG_THRESHOLD_SRC = Analog sensor, 0x10FB[2:1] = "10"
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CFG1, SMB_MASK(2, 1), 0x4);
	if (rc)
		pr_err("set TEWM_I_SRC = FUEL GAUGE_ADC, RECHG_THRESHOLD_SRC = Analog sensor failed\n");
	// SMBCHGL_USB_CFG.HVDCP_ADAPTER_SEL = HVDCP_5V, 0x13F4[5:4] = "00"
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base+ CHGPTH_CFG, HVDCP_ADAPTER_SEL_MASK, 0x0);
	if (rc)
		pr_err("set HVDCP adapter sel = 5V failed\n");

	// SET CHARGER EN =REGISTER CONTROL
	// CHG_EN_COMMAND= Active low
	// BATT_OV_ENDS_CYCLE_DIS
	// EN AUTO RECHARGH && PRE_FAST_AUTO
	// HOLDOFF_TMR_700US
	// 0x10FC[7:0]= 0x42
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CFG2, SMB_MASK(7, 0), 0x42);
	if (rc)
		pr_err("set 0x10FC failed\n");
	// EN_BAT_CHG =DISABLE, 0x1242[1] = "1"
	rc = smbchg_charging_en(chip, false);
	if (rc)
		pr_err("Disabling charging failed\n");
	// EN_BAT_CHG =ENABLE, 0x1242[1] = "0"
	rc = smbchg_charging_en(chip, true);
	if (rc)
		pr_err("Enabling charging failed\n");

	// SMBCHGL_BAT_IF_CFG_SYSMIN = SYSMIN_3P6, 0x12F4[1:0] = 0x02
	rc = smbchg_sec_masked_write(chip, chip->bat_if_base+ CFG_SYSMIN_REG, SMB_MASK(1, 0), 0x2);
	if (rc)
		pr_err("set minimum system voltage settings = 3.6V  failed\n");
	chip->asus_thermal_policy_flag = asus_thermal_level0;
	chip->asus_dual_flag = Dual_None;
	chip->asus_show_qc_flag = 0;
	chip->asus_hvdcp_flag = 0;
	...
	ASUS_CHG_TYPE = "UNDEFINED";
	asus_force_hvdcp_setting(chip, false);
	asus_smb1351_initial_setting();

}

static void asus_read_ICL_mode(struct smbchg_chip *chip, u8 *val)
{
	int rc;
	u8 reg;
	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + ICL_STS_2_REG, 1);
	if (rc) {
		pr_err("read ICL Mode failed, default 100mA\n");
		*val = 0x1;
		return;
	}
	reg &= (BIT(5) | BIT(4));
	...
	*val = reg;
	pr_smb(PR_STATUS,
					"ICL Mode = 0x%x, %s\n", *val, (*val == 0x0 ? "HC" : (*val == 0x01 ? "100 mA": (*val == 0x2 ? "500 mA" : "Reserved"))));
}
...
int asus_otg_boost_enable(int enable, bool high_current)
{
	...
	if (enable) {
		...
		*/
		...
		smbchg_usb_suspend(chip, true);
		asus_smb1351_boost_enable(enable, high_current, chip->asus_is_otg);
		chip->asus_is_otg = true;
	} else {
		if (!chip->asus_is_otg)
			return 0;
		chip->asus_is_otg = false;
		asus_smb1351_boost_enable(enable, high_current, false);
...
EXPORT_SYMBOL(asus_otg_boost_enable);
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (chip->asus_hvdcp_rerun_flag == 2) {
		chip->asus_hvdcp_rerun_flag = 0;
		msleep(3000);
		if (!is_usb_present(chip)) {
			asus_JEITA_rule(chip);
		...
		pr_info("Charging with HVDCP: 9V, QC %s adaptor\n",
				(chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) ? "3.0" : "2.0");
		if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
			asus_auto_pulse_up_to_9V(chip);
		else
			asus_force_hvdcp_setting(chip, true);
	...
	cancel_delayed_work(&chip->asus_JEITA_check_work);

	asus_initial_settings(chip);
	if (!asus_wake_lock_dis)
		wake_lock(&chip->asus_adapter_detect_wake_lock);
	/* usb inserted */
	read_usb_type(chip, &usb_type_name, &usb_supply_type);
	pr_smb(PR_STATUS,
		"inserted type = %d (%s)", usb_supply_type, usb_type_name);
	chip->asus_cable_online = true;
	smbchg_aicl_deglitch_wa_check(chip);
#ifdef PD_CHARGING_DRIVER_SUPPORT
	if (!strcmp(usb_type_name, "OTHER"))
		asus_phy_chg_detect();
#endif
	...
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP && strcmp("OTHER", usb_type_name)) {
...
static int smbchg_unprepare_for_pulsing_lite(struct smbchg_chip *chip)
{
	...
	pr_info("Forcing 5V and clear ADC...\n");
	gpio_set_value(chip->adcpwren_gpio, 0);
	gpio_set_value(chip->adc_sw_en_gpio, 0);
	asus_force_hvdcp_setting(chip, false);
	if (is_usb_present(chip)) {
		if (!is_hvdcp_present(chip)) {
			pr_err("hvdcp gone, retry......\n");
			chip->hvdcp_3_det_ignore_uv = true;
		...
		else if (!chip->allow_hvdcp3_detection)
			smbchg_handle_hvdcp3_disable(chip);
	}
...
static void asus_auto_pulse_up_to_9V(struct smbchg_chip *chip)
{
	int i, rc;
	if (chip->pulse_cnt != 0) {
		pr_err("pulse_cnt != 0 !!!!, clear and reset to 0\n");
		chip->pulse_cnt = 0;
	}
	for (i = 0; i < 20; i++) {
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if ((get_prop_batt_capacity(chip) == 100 && chip->asus_cable_online)
			...
			if (chip->asus_cable_online && battery_status != POWER_SUPPLY_STATUS_FULL)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
			else
				val->intval = get_prop_batt_status(chip);
		}
		if (g_charger_mode && get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_FULL) {
			rc = smbchg_read(chip, &reg, chip->chgr_base + FV_CFG_REG, 1);
			if (rc) {
				pr_err("read Vfloat settings failed\n");
				reg = 0;
			}
			reg &= FV_CFG_MASK;
			if (reg == 0x2D) {
				if (wake_lock_active(&chip->asus_adapter_detect_wake_lock)) {
					pr_info("unlock adapter detect wake lock when full charge at 4.38V\n");
					wake_unlock(&chip->asus_adapter_detect_wake_lock);
				}
			}
		}
...
#define OTG_ICFG_REG 0xF3
static void asus_power_on_settings(struct smbchg_chip *chip){
	int rc;
	// SMBCHGL_OTG_OTG_CFG, 0x11F1 = 0x28
	rc = smbchg_sec_masked_write(chip, chip->otg_base+ OTG_OC_CFG, 0xFF, 0x28);
	if (rc)
		pr_err("set SMBCHGL_OTG_OTG_CFG = 0x28 failed\n");
	// SMBCHG_OTG_CFG_BATTUV=2.9V, 0x11F2 = 0x01
	rc = smbchg_sec_masked_write(chip, chip->otg_base+ OTG_CFG_BATTUV, 0x3, 0x01);
	if (rc)
		pr_err("set OTG_BATTUV = 2.9V failed\n");
	// SMBCHGL_OTG_OTG_ICFG = OTG_ILIMIT_250MA, 0x11F3 = 0x00
	rc = smbchg_sec_masked_write(chip, chip->otg_base+ OTG_ICFG_REG, 0x3, 0x0);
	if (rc)
		pr_err("set OTG output current limit = 250 mA failed\n");
}
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	chip->adcpwren_gpio= of_get_named_gpio_flags(node,
		"ADCPWREN-gpios01", 0, &flags);
	chip->adc_sw_en_gpio= of_get_named_gpio_flags(node,
		"ADC_SW_EN", 0, &flags);
	return 0;
}
...
static int create_debugfs_entries(struct smbchg_chip *chip)
{
	...
	ent = debugfs_create_file("dump_reg", S_IFREG | S_IRUGO,
				chip->debug_root, chip,
				&asus_dump_debugfs_ops);
	if (!ent) {
		dev_err(chip->dev,
	...
	debugfs_create_file("battery_current_now", S_IFREG | S_IWUSR | S_IRUGO,
				chip->debug_root, chip,
				&asus_battery_current_now_ops);
	debugfs_create_file("CHG_TYPE_now", S_IFREG | S_IWUSR | S_IRUGO,
				chip->debug_root, chip,
				&asus_CHG_TYPE_now_ops);
	debugfs_create_file("asus_force_QC30", S_IFREG | S_IWUSR | S_IRUGO,
				chip->debug_root, chip,
				&asus_force_QC30_ops);

	debugfs_create_file("asus_thermal_t1", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_thermal_t1_ops);
	debugfs_create_file("asus_thermal_t2", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_thermal_t2_ops);
	debugfs_create_file("asus_thermal_t3", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_thermal_t3_ops);
	debugfs_create_file("asus_ftm_force_usb_suspend", S_IFREG | S_IWUSR | S_IRUGO,
				chip->debug_root, chip,
				&asus_force_usb_suspend_ops);
	if (g_ftm_mode) {
		debugfs_create_file("asus_ftm_force_2A", S_IFREG | S_IWUGO | S_IRUGO,
					chip->debug_root, chip,
					&asus_ftm_force_2A_ops);
		debugfs_create_file("asus_charger_limit_enable", S_IFREG | S_IWUGO | S_IRUGO,
					chip->debug_root, chip,
					&asus_charger_limit_enable_ops);
		debugfs_create_file("asus_charger_limit_percentage", S_IFREG | S_IWUGO | S_IRUGO,
					chip->debug_root, chip,
					&asus_charger_limit_perc_ops);
	} else {
		debugfs_create_file("asus_ftm_force_2A", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_ftm_force_2A_ops);
		debugfs_create_file("asus_charger_limit_enable", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_charger_limit_enable_ops);
		debugfs_create_file("asus_charger_limit_percentage", S_IFREG | S_IWUSR | S_IRUGO,
					chip->debug_root, chip,
					&asus_charger_limit_perc_ops);
	}
	return 0;
...
static void asus_set_high_usb_chg_current(struct smbchg_chip *chip, int current_ma)
{
	int i, rc, usb_cur_val;

	i = find_smaller_in_array(chip->tables.usb_ilim_ma_table,
			current_ma, chip->tables.usb_ilim_ma_len);
	if (i < 0) {
		pr_err("Cannot find %dma current_table using %d\n",
			current_ma, CURRENT_500_MA);
		i = 0x4;
	}
	...
	// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
	rc = smbchg_masked_write(chip, chip->usb_chgpth_base + CMD_IL,
		SMB_MASK(6, 0), USBIN_HC_MODE | BIT(2));
	if (rc < 0)
		pr_err("set USBIN = HC Mode failed\n");
	// SMBCHGL_USB_USBIN_IL_CFG, 0x13F2
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
		USBIN_INPUT_MASK, usb_cur_val);
	if(rc)
		pr_err("set USBIN input limit = %d mA failed\n", current_ma);
	// re-run AICL
	smbchg_rerun_aicl(chip);
	pr_info("setting USBIN input limit = %d mA, reg = 0x%x\n", current_ma, usb_cur_val);
}

#ifdef PD_CHARGING_DRIVER_SUPPORT
bool asus_is_low_battery(void)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	if (asus_smbchg_probe_done) {
		if (get_prop_batt_capacity(chip) < 1)
			return true;
		else
			return false;
	} else {
		pr_err("driver not ready\n");
		return true;
	}
}
EXPORT_SYMBOL(asus_is_low_battery);

bool asus_is_chg_high_temp(void)
{
	int chg_temp;
	int chg1_temp, chg2_temp;
	if (asus_smbchg_probe_done) {
		chg1_temp = us5587_read_pmi8952_thermal();
		chg2_temp = us5587_read_smb1351_thermal();

		chg_temp = (chg1_temp >= chg2_temp) ? chg1_temp : chg2_temp;
		if (chg_temp >= asus_therm_t1)
			return true;
		else
			return false;
	} else {
...
EXPORT_SYMBOL(asus_is_chg_high_temp);

void asus_smbchg_usbin_suspend(bool enable)
{
	struct smbchg_chip *chip = g_smbchg_chip;
	smbchg_usb_suspend(chip, enable);
}
EXPORT_SYMBOL(asus_smbchg_usbin_suspend);

static void asus_typec_DFP_setting(struct smbchg_chip *chip)
{
	int rc;
	u8 reg;
	enum power_supply_type usb_supply_type;
	...
	pr_info("DFP Setting for type: %d, max voltage: %d mV, max input current: %d mA\n",
				chip->asus_typec_chg, chip->asus_pd_volt, chip->asus_pd_curr);
	switch (chip->asus_typec_chg) {
		case TYPEC_1_5A_CHARGER:
			ASUS_CHG_TYPE = "TYPEC_1P5A";
			// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
			// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_1400MA, 0x13F2 = 0xE
			// re-run AICL
			...
			// AICL done(0x1307[5]) && result(0x1307[4:0]) <= 1200mA
			rc = smbchg_read(chip, &reg, chip->usb_chgpth_base+ ICL_STS_1_REG, 1);
			if (rc ||
				((reg & AICL_STS_BIT) && ((reg & ICL_STS_MASK) <= 0xD))) {
				if (rc)
					pr_info("read AICL status failed\n");
				else
					pr_info("AICL <= 1200 mA\n");
				read_usb_type(chip, &usb_type_name, &usb_supply_type);
				if (usb_supply_type == POWER_SUPPLY_TYPE_USB) {
					// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_500MA, 0x13F2 = 0x04
					rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
						USBIN_INPUT_MASK, 0x4);
					if(rc)
						pr_err("set USBIN input limit = 500 mA failed\n");
				} else {
					// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_900MA, 0x13F2 = 0x09
					rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
						USBIN_INPUT_MASK, 0x9);
					if(rc)
						pr_err("set USBIN input limit = 900 mA failed\n");
				}
			}
			chip->asus_dual_flag = Dual_single;
			break;
		case TYPEC_3A_CHARGER:
			ASUS_CHG_TYPE = "TYPEC_3P0A";
			// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
			// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_1400MA, 0x13F2 = 0xE
			// re-run AICL
			asus_set_high_usb_chg_current(chip, 1400);
			...
			// AICL done(0x1307[5]) && result(0x1307[4:0]) <= 500mA
			rc = smbchg_read(chip, &reg, chip->usb_chgpth_base+ ICL_STS_1_REG, 1);
			if (rc ||
				((reg & AICL_STS_BIT) && ((reg & ICL_STS_MASK) <= 0x4))) {
				if (rc)
					pr_info("read AICL status failed\n");
				else
					pr_info("AICL <= 500 mA\n");
				...
				chip->asus_dual_flag = Dual_TYPEC_3A;
				// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_900MA, 0x13F2 = 0x09
				rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
					USBIN_INPUT_MASK, 0x9);
				if(rc)
		...
		case TYPEC_PD_9V_CHARGER:
			ASUS_CHG_TYPE = "PD";
			pr_info("setting PD charging. type: %d, current: %d mA, voltage: %d mV\n",
				chip->asus_typec_chg, chip->asus_pd_curr, chip->asus_pd_volt);
			if (chip->asus_pd_curr == 1000 || chip->asus_pd_curr == 1500)
				chip->asus_dual_flag = Dual_single;
			else if (chip->asus_pd_curr == 2000)
				chip->asus_dual_flag = Dual_ASUS_2A;
			else if (chip->asus_pd_curr == 3000)
				chip->asus_dual_flag = Dual_TYPEC_3A;
			else
				pr_err("Illegal PD profile: type: %d, current: %d mA, voltage: %d mV\n",
					chip->asus_typec_chg, chip->asus_pd_curr, chip->asus_pd_volt);

			if (chip->asus_typec_chg >= TYPEC_PD_5V_CHARGER) {
				msleep(500);
				smbchg_usb_suspend(chip, false);
				if (chip->asus_dual_flag == Dual_single) {
					if (chip->asus_pd_curr == 1500) {
						// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
						// SMBCHGL_USB_USBIN_IL_CFG , 0x13F2 = 1400 mA
						// re-run AICL
						asus_set_high_usb_chg_current(chip, 1400);
					} else {
						// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
						// SMBCHGL_USB_USBIN_IL_CFG , 0x13F2 = 900 mA
						// re-run AICL
						asus_set_high_usb_chg_current(chip, 900);
					}
				} else if (chip->asus_dual_flag == Dual_ASUS_2A) {
					// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
					// SMBCHGL_USB_USBIN_IL_CFG , 0x13F2 = 900 mA
					// re-run AICL
					asus_set_high_usb_chg_current(chip, 900);
...
int asus_typec_DFP_type(enum typec_power_supply_type type, int voltage_mv, int current_ma){
	struct smbchg_chip *chip;
	if (g_smbchg_chip == NULL || !asus_smbchg_probe_done) {
		pr_err("driver not ready...type: %d\n", type);
		if (type != TYPEC_UNKNOWN_CHARGER)
			DFP_notify = true;
		return -1;
	}

	chip = g_smbchg_chip;
	if (chip->asus_cc_wa_flag && type <= TYPEC_3A_CHARGER) {
		pr_info("ignore incorrect DFP type reported by CC\n");
		return 0;
	}
	if (type == TYPEC_UNKNOWN_CHARGER) {
		pr_info("ignore unknown DFP type reported by CC\n");
		return 0;
	}
	if (is_hvdcp_present(chip)) {
		pr_info("ignore DFP type if hvdcp present\n");
		return 0;
	}
	pr_info("%sgetting DFP type from CC. type: %d, voltage: %d mV, current: %d mA\n",
		(chip->asus_typec_chg != type)? "":"Duplicated, ",type, voltage_mv, current_ma);
	if (chip->asus_typec_chg != type) {
		cancel_delayed_work_sync(&chip->asus_bc_1_2_det_work);
		cancel_delayed_work_sync(&chip->asus_adapter_detect_work);
		pr_info("setting new DFP type: %d, voltage: %d mV, current: %d mA\n", type, voltage_mv, current_ma);
		chip->asus_pd_volt = voltage_mv;
		if (current_ma == 1500 && voltage_mv == 5000)
			chip->asus_pd_curr =1500;
		else if (current_ma >= 1000 && current_ma < 2000)
			chip->asus_pd_curr = 1000;
		else if (current_ma >= 2000 && current_ma < 3000)
			chip->asus_pd_curr = 2000;
		else if (current_ma >= 3000)
			chip->asus_pd_curr = 3000;
		else
			chip->asus_pd_curr = 0;
		chip->asus_typec_chg = type;
		chip->asus_cc_wa_flag = 1;
		asus_initial_settings(chip);
		queue_delayed_work(smbchg_wq, &chip->asus_bc_1_2_det_work, msecs_to_jiffies(5000));
	}
	return 0;
}
EXPORT_SYMBOL(asus_typec_DFP_type);
#endif

enum JEITA_CURRENT_LEVEL {
	jeita_1200mA = 0,
	jeita_2000mA,
	jeita_2100mA,
};

static void asus_JEITA_config1(struct smbchg_chip *chip) {
	int rc;
	pr_info("setting usb input limit current: 1910 mA\n");
	// INPUT CURRENT_LIMIT = USBIN_IL_1910MA
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
		USBIN_INPUT_MASK, 0x15);
	if(rc)
		pr_err("set USBIN input limit = 1910 mA failed\n");
	// re-run AICL
	smbchg_rerun_aicl(chip);
}
static void asus_JEITA_config2(struct smbchg_chip *chip, bool chg_en, bool high_chg_fv, int fcc_cur_level) {
	int rc;
	rc = smbchg_charging_en(chip, chg_en);
	if (rc)
		pr_err("%s charging failed\n", chg_en ? "Enabling" : "Disabling");
	...
	if (high_chg_fv) {
		...
		// SMBCHGL_CHGR_FV_CFG = FV_4P1, 0x10F4 = 0x1E
		rc = smbchg_sec_masked_write(chip, chip->chgr_base + FV_CFG_REG, FV_CFG_MASK, 0x1E);
		if (rc)
			pr_err("set Float Voltage =  4.1V failed\n");
	}

	if (fcc_cur_level == jeita_2100mA) {
		// SMBCHGL_CHGR_FCC_CFG = FCC_2100MA, 0x10F2= 0x1B
		rc = smbchg_sec_masked_write(chip, chip->chgr_base + FCC_CFG_REG, FCC_CFG_MASK, 0x1B);
		if (rc)
			pr_err("set Fast-charge current = 2100 mA failed\n");
	} else if (fcc_cur_level == jeita_2000mA) {
		// SMBCHGL_CHGR_FCC_CFG = FCC_2000MA, 0x10F2= 0x19
		rc = smbchg_sec_masked_write(chip, chip->chgr_base + FCC_CFG_REG, FCC_CFG_MASK, 0x19);
		if (rc)
			pr_err("set Fast-charge current = 2000 mA failed\n");
	} else {
		// SMBCHGL_CHGR_FCC_CFG = FCC_1200MA, 0x10F2= 0x0D
		rc = smbchg_sec_masked_write(chip, chip->chgr_base + FCC_CFG_REG, FCC_CFG_MASK, 0xD);
		if (rc)
			pr_err("set Fast-charge current = 1200 mA failed\n");
	}
}

static void asus_recharge_func(struct smbchg_chip *chip) {
	int rc, soc;
	u8 reg;
	soc = get_prop_batt_capacity(chip);
	rc = smbchg_read(chip, &reg, chip->chgr_base + CHGR_STS, 1);
...
#ifdef PD_CHARGING_DRIVER_SUPPORT
static void asus_thermal_policy_pd(struct smbchg_chip *chip)
{
	usbpd_jump_voltage();
	asus_Dual_Disable();
}

static void asus_thermal_policy_typec(struct smbchg_chip *chip)
{
	asus_set_high_usb_chg_current(chip, 1400);
	asus_Dual_Disable();
}
...
static void asus_thermal_policy_10w(struct smbchg_chip *chip)
{
	asus_set_high_usb_chg_current(chip, 900);
	asus_Dual_Disable();
}

static void asus_thermal_policy_qc20(struct smbchg_chip *chip)
{
	chip->asus_thermal_policy_flag = asus_thermal_level0;
	asus_force_hvdcp_setting(chip, false);
}

static void asus_thermal_policy_qc30(struct smbchg_chip *chip, bool is_hot)
{
	int i, rc;
	chip->asus_thermal_policy_flag = asus_thermal_level0;
	if (is_hot) {
...
static void asus_thermal_policy_level1(struct smbchg_chip *chip)
{
	pr_info("thermal Level 1\n");
	if (chip->asus_thermal_policy_flag == asus_thermal_level3) {
		smbchg_usb_suspend(chip, false);
	}
	chip->asus_thermal_policy_flag = asus_thermal_level1;
#ifdef PD_CHARGING_DRIVER_SUPPORT
	if (chip->asus_typec_chg >= TYPEC_PD_5V_CHARGER)
		asus_thermal_policy_pd(chip);
	else {
#endif
...
#ifdef PD_CHARGING_DRIVER_SUPPORT
			case Dual_TYPEC_3A:
				asus_thermal_policy_typec(chip);
				break;
#endif
			default:
				pr_err("unknown type for thermal policy\n");
				break;
		};
#ifdef PD_CHARGING_DRIVER_SUPPORT
	}
...
static void asus_thermal_policy_level2(struct smbchg_chip *chip)
{
	int batt_cap = get_prop_batt_capacity(chip);
	pr_info("thermal Level 2\n");
...
{
	pr_info("thermal Level 3\n");
	chip->asus_thermal_policy_flag = asus_thermal_level3;
	smbchg_usb_suspend(chip, true);
	asus_Dual_Disable();
}

static void asus_thermal_policy_reset(struct smbchg_chip *chip)
{
		...
		if (chip->asus_typec_chg >= TYPEC_PD_5V_CHARGER) {
			...
			switch (chip->asus_dual_flag) {
				case Dual_single:
					if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
							asus_set_high_usb_chg_current(chip, 900);
							asus_thermal_policy_qc30(chip, false);
					} else if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
						asus_set_high_usb_chg_current(chip, 900);
						asus_force_hvdcp_setting(chip, true);
					}
...
#endif
	} else if (chip->asus_thermal_policy_flag == asus_thermal_level0 && chip->asus_cable_online) {
		if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
			asus_set_high_usb_chg_current(chip, 900);
			asus_thermal_policy_qc30(chip, false);
		}
...
static void asus_thermal_policy(struct smbchg_chip *chip) {
	int chg_temp;
	...
	if (!chip->asus_cable_online) {
		pr_info("no cable connected...reset to level 0\n");
		chip->asus_thermal_policy_flag = asus_thermal_level0;
		return;
	} else if (chip->asus_dual_flag == Dual_None) {
		pr_info("cable present but battery level is low\n");
	} else if (chg_temp < asus_therm_t1) {
		asus_thermal_policy_reset(chip);
	} else if (chg_temp >= asus_therm_t1 && chg_temp < asus_therm_t2)
		asus_thermal_policy_level1(chip);
	else if (chg_temp >= asus_therm_t2 && chg_temp < asus_therm_t3)
		asus_thermal_policy_level2(chip);
	else if (chg_temp >= asus_therm_t3)
		asus_thermal_policy_level3(chip);

	queue_delayed_work(smbchg_wq, &chip->asus_thermal_policy_work, msecs_to_jiffies(10500));
}

static int asus_get_prop_batt_FCC(struct smbchg_chip *chip)
{
	int FCC, rc;
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_CHARGE_FULL, &FCC);
	if (rc) {
		pr_err("Couldn't get POWER_SUPPLY_PROP_CHARGE_FULL\n");
		return 4490;
	}
	return FCC/1000;
}
...
static void asus_JEITA_rule(struct smbchg_chip *chip) {
	int rc, batt_temp, batt_volt, batt_cap, batt_curr, batt_fcc;
	u8 reg;
	bool is_high_cur;
	bool is_high_vchg;
	// check stop charging criteria for eng

	if (asus_wake_lock_dis && wake_lock_active(&chip->asus_adapter_detect_wake_lock))
		wake_unlock(&chip->asus_adapter_detect_wake_lock);
	if (chip->psy_registered)
			power_supply_changed(&chip->batt_psy);
	cancel_delayed_work_sync(&chip->asus_thermal_policy_work);
	asus_thermal_policy(chip);
	...
	batt_fcc = asus_get_prop_batt_FCC(chip);

	if (chip->usb_supply_type != POWER_SUPPLY_TYPE_USB) {
		// AICL done(0x1307[5])
		rc = smbchg_read(chip, &reg, chip->usb_chgpth_base+ ICL_STS_1_REG, 1);
		if (!rc)
			pr_info("AICL%s done!! ICL sts : %d mA\n", (reg & AICL_STS_BIT) ? "": " not", chip->tables.usb_ilim_ma_table[reg & ICL_STS_MASK]);
	} else {
		rc = smbchg_read(chip, &reg, chip->usb_chgpth_base+ CMD_IL, 1);
		if (!rc)
			pr_info("USB type, %sHC mode\n", (reg & USBIN_MODE_CHG_BIT) ? "" : "not ");
	}

	if (chip->asus_is_otg) {
		// pet smb1351 watchdog to keep vbus power
		pr_info("otg connected...");
		if (g_reverse_charger_mode)
			asus_otg_boost_enable(1, true);
		else
			asus_otg_boost_enable(1, false);
	...
	if (!is_usb_present(chip)) {
		pr_info("soc: %d%%, voltage: %d mV, temp: %d C, current: %d mA, last_temp_state: %d\n",
			...
			-batt_curr/1000, chip->asus_batt_last_state);
		ASUSEvtlog("[BAT][Ser]report Capacity ==>%d, FCC:%dmAh, BMS:%d, V:%dmV, Cur:%dmA, Temp:%dC\n",
			batt_cap, batt_fcc, batt_cap, batt_volt/1000, -batt_curr/1000, batt_temp/10);
		pr_info("usb removed!! bypass JEITA check\n");
		queue_delayed_work(smbchg_wq, &chip->asus_JEITA_check_work, msecs_to_jiffies(60000));
		return;
	}

	wake_lock(&chip->asus_jeita_setting_wake_lock);
	mutex_lock(&chip->asus_jeita_setting_lock);
	asus_smb1351_JEITA_rule();
	// JEITA_TEMP_HARD_LIMIT = JEITA_TEMP_HARD_LIMIT_EN, 0x10FA[5] = "0"
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CCMP_CFG, JEITA_TEMP_HARD_LIMIT_BIT, 0x0);
	if (rc)
		pr_err("set JEITA temp hard limit enabled failed\n");
	// HOT_SOFT_LIMIT_FV_COMP_DIS
	// COLD_SOFT_LIMIT_FV_COMP_DIS
	// HOT_SOFT_LIMIT_CC_COMP_DIS
	// COLD_SOFT_LIMIT_CC_COMP_DIS
	// 0x10FA[3:0] = "0000"
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CCMP_CFG, SMB_MASK(3, 0), 0x0);
	if (rc)
		pr_err("config JEITA compensation disable failed\n");

	if (chip->asus_batt_last_state== less_than_0 &&
		batt_temp <= 30)
		batt_temp = -1;
	else if (chip->asus_batt_last_state== from_0_to_100 &&
		batt_temp <= 130 && batt_temp >= 100)
		batt_temp = 90;
	else if (chip->asus_batt_last_state== from_100_to_200 &&
		batt_temp <= 230  && batt_temp >= 200)
		batt_temp = 190;
	else if (chip->asus_batt_last_state == from_500_to_600 &&
		batt_temp >= 470  && batt_temp < 500)
		batt_temp = 540;
	else if (chip->asus_batt_last_state == greater_than_600 &&
		batt_temp >= 570)
		batt_temp = 650;
	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base+ ICL_STS_1_REG, 1);
	if (rc) {
		pr_err("read AICL status failed\n");
		reg =  BIT(5);
	}
	reg = reg & SMB_MASK(4, 0);
	if (reg == 0x9 && chip->asus_dual_flag >= Dual_ASUS_2A && !chip->asus_hvdcp_flag)
		is_high_cur = true;
	else
		is_high_cur = false;
	...
	if (g_ftm_mode && asus_charger_limit_en) {
		if ((asus_charger_limit_percentage > batt_cap + 19 && asus_charger_limit_percentage >= 20) ||
	...
	if (batt_temp < 0) {
		chip->asus_batt_last_state = less_than_0;
		asus_Dual_Disable();
		if (is_high_cur && chip->asus_thermal_policy_flag == asus_thermal_level0)
			asus_JEITA_config1(chip);
		asus_JEITA_config2(chip, false, true, jeita_1200mA);
	} else if (batt_temp >= 0 && batt_temp < 100) {
		chip->asus_batt_last_state = from_0_to_100;
		asus_Dual_Disable();
		if (is_high_cur && chip->asus_thermal_policy_flag == asus_thermal_level0)
			asus_JEITA_config1(chip);
		asus_JEITA_config2(chip, true, true, jeita_1200mA);
		asus_recharge_func(chip);
	} else if (batt_temp >= 100 && batt_temp < 200) {
		chip->asus_batt_last_state = from_100_to_200;
		asus_Dual_Disable();
		if (is_high_cur && chip->asus_thermal_policy_flag == asus_thermal_level0)
			asus_JEITA_config1(chip);
		asus_JEITA_config2(chip, true, true, jeita_2100mA);
		asus_recharge_func(chip);
	} else if (batt_temp >= 200 && batt_temp < 500) {
		chip->asus_batt_last_state = from_200_to_500;
		if (chip->asus_dual_flag <= Dual_single || chip->asus_thermal_policy_flag != asus_thermal_level0) {
			asus_Dual_Disable();
			asus_JEITA_config2(chip, true, true, jeita_2100mA);
			asus_recharge_func(chip);
		} else if (chip->asus_dual_flag == Dual_ASUS_2A && chip->asus_thermal_policy_flag == asus_thermal_level0) {
			if (batt_volt >= 3000000 && batt_volt <= 4300000) {
				asus_JEITA_config2(chip, true, true, jeita_2000mA);
				// CURRENT_LIMIT = USBIN_IL_900MA, 0x13F2[4:0] = 0x09
			...
			asus_recharge_func(chip);
		} else if (chip->asus_dual_flag == Dual_TYPEC_3A && chip->asus_thermal_policy_flag == asus_thermal_level0) {
			if (batt_volt >= 3000000 && batt_volt <= 4300000) {
				asus_JEITA_config2(chip, true, true, jeita_2000mA);
				// CURRENT_LIMIT = USBIN_IL_1400MA, 0x13F2[4:0] = 0x0E
				smbchg_sec_masked_write(chip, chip->usb_chgpth_base + IL_CFG,
						USBIN_INPUT_MASK, 0xE);
				asus_Dual_Enable(1500, 2000);
			} else {
				asus_Dual_Disable();
				if (is_high_cur)
	...
	} else if (batt_temp >= 500 && batt_temp < 600) {
		chip->asus_batt_last_state = from_500_to_600;
		asus_Dual_Disable();
		if (is_high_cur && chip->asus_thermal_policy_flag == asus_thermal_level0)
			asus_JEITA_config1(chip);
		if (is_high_vchg && batt_volt >= 4100000)
			asus_JEITA_config2(chip, false, true, jeita_2100mA);
		else
			asus_JEITA_config2(chip, true, false, jeita_2100mA);
	} else if (batt_temp >= 600) {
		chip->asus_batt_last_state = greater_than_600;
		asus_Dual_Disable();
		if (is_high_cur && chip->asus_thermal_policy_flag == asus_thermal_level0)
			asus_JEITA_config1(chip);
		asus_JEITA_config2(chip, false, true, jeita_2100mA);
	} else
		pr_err("Unknown battery temperature!!\n");
...
#ifdef PD_CHARGING_DRIVER_SUPPORT
	pr_info("type C adapter : %d, %d mV, %d mA\n", chip->asus_typec_chg, chip->asus_pd_volt, chip->asus_pd_curr);
	if (DFP_notify) {
		pr_info("CC tried to set DFP type before...notify CC to rerun\n");
		usbpd_jump_voltage();
		DFP_notify = false;
	}
#endif

	mutex_unlock(&chip->asus_jeita_setting_lock);
	wake_unlock(&chip->asus_jeita_setting_wake_lock);
	queue_delayed_work(smbchg_wq, &chip->asus_JEITA_check_work, msecs_to_jiffies(60000));
}

static void asus_adapter_detect_func(struct work_struct *work) {
	...
	if (asus_charger_force_2A) {
		pr_info("force setting dula flag as 2A\n");
		chip->asus_dual_flag = Dual_ASUS_2A;
		goto out;
	}
	...
	gpio_set_value(chip->adcpwren_gpio, 0);
	gpio_set_value(chip->adc_sw_en_gpio, 1);
	// PMI charger input suspend, 0x1340[4] = "1"
	smbchg_usb_suspend(chip, true);
	msleep(5);

	adc_result = us5587_dump_value();
	if (adc_result <= 0x3F) {
		gpio_set_value(chip->adcpwren_gpio, 1);
		msleep(5);
		adc_result = us5587_dump_value();
		if (adc_result > 0xD4) {
			chip->asus_dual_flag = Dual_single;
			...
			if (adc_result >= 0x98 && adc_result <= 0xAD) {
				if (!chip->asus_hvdcp_flag) {
					chip->asus_dual_flag = Dual_ASUS_2A;
					ASUS_CHG_TYPE = "DCP_ASUS_750K_2A";
				}
				else {
					chip->asus_dual_flag = Dual_single;
					ASUS_CHG_TYPE = "HVDCP_OTHERS_1A";
				}
			} else if (adc_result >= 0x3F && adc_result <= 0x54) {
				if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
					chip->asus_dual_flag = Dual_ASUS_2A;
					ASUS_CHG_TYPE = "HVDCP_ASUS_200K_2A";
				} else {
					chip->asus_dual_flag = Dual_single;
					ASUS_CHG_TYPE = "HVDCP_OTHERS_1A";
				}
	...
	// PMI charger input work, 0x1340[4] = "0"
	smbchg_usb_suspend(chip, false);
	gpio_set_value(chip->adc_sw_en_gpio, 0);
	gpio_set_value(chip->adcpwren_gpio, 0);

	if (chip->asus_hvdcp_flag || chip->asus_dual_flag == Dual_ASUS_2A) {
		if (is_hvdcp_present(chip)) {
			if (chip->asus_dual_flag == Dual_ASUS_2A) {
				// this is only for asus 200K adapter.
				chip->asus_show_qc_flag = 1;
				if (chip->psy_registered)
					power_supply_changed(&chip->batt_psy);
			}
			// re-run apsd
			chip->asus_hvdcp_rerun_flag = 1;
			rc = rerun_apsd(chip);
			if (rc) {
				pr_err("APSD rerun failed\n");
				chip->asus_hvdcp_rerun_flag = 0;
...
static void asus_update_battery_func(struct work_struct *work) {
	struct smbchg_chip *chip = container_of(work, struct smbchg_chip,
		asus_update_battery_work.work);
	int batt_temp, batt_volt, batt_cap, batt_curr, batt_fcc;

	wake_lock(&chip->asus_update_battery_wake_lock);
	if (chip->psy_registered)
			power_supply_changed(&chip->batt_psy);
	...
	if(wake_lock_active(&chip->asus_update_battery_wake_lock))
		wake_unlock(&chip->asus_update_battery_wake_lock);
}

static void asus_recheck_dcp_setting(struct smbchg_chip *chip) {
...
#endif
		// ASUS Adapter Detect
		if (chip->asus_hvdcp_flag == 0) {
			pr_info("switching ADC channel...\n");
			gpio_set_value(chip->adcpwren_gpio, 0);
			gpio_set_value(chip->adc_sw_en_gpio, 1);
			queue_delayed_work(smbchg_wq, &chip->asus_adapter_detect_work, msecs_to_jiffies(30000));
		}
		else
			queue_delayed_work(smbchg_wq, &chip->asus_adapter_detect_work, msecs_to_jiffies(100));
	}
}

int asus_force_reset_as_CDP(void)
{
	struct smbchg_chip *chip;
	enum power_supply_type usb_supply_type;
	char *usb_type_name = "NULL";
	...
	if (asus_smbchg_probe_done) {
		chip = g_smbchg_chip;
		read_usb_type(chip, &usb_type_name, &usb_supply_type);
		if (!strcmp("OTHER", usb_type_name)) {
			pr_info("BC 1.2 detecting OTHER type...clear previous queue...\n");
			cancel_delayed_work_sync(&chip->asus_bc_1_2_det_work);
			cancel_delayed_work_sync(&chip->asus_adapter_detect_work);
			cancel_delayed_work_sync(&chip->asus_thermal_policy_work);
			cancel_delayed_work_sync(&chip->asus_JEITA_check_work);
			pr_info("force set 1400mA to charge as CDP port\n");
			// USBIN_MODE_CHG = HC Mode Current Level, 0x1340[7:0] = 0x05
			// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_900MA, 0x13F2[4:0] = 0x09
			asus_set_high_usb_chg_current(chip, 900);
#ifdef PD_CHARGING_DRIVER_SUPPORT
			// check DFP type
			if (chip->asus_typec_chg > TYPEC_PD_NO_READY_CHARGER)
				asus_typec_DFP_setting(chip);
			else {
#endif
				pr_info("setting usb input limit current: 1400 mA\n");
				// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
				// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_1400MA, 0x13F2 = 0xE
				// re-run AICL
				asus_set_high_usb_chg_current(chip, 1400);
				chip->asus_dual_flag = Dual_single;
				ASUS_CHG_TYPE = "CDP_1P5A";
#ifdef PD_CHARGING_DRIVER_SUPPORT
			}
#endif
			asus_JEITA_rule(chip);
			...
			pr_err("BC 1.2 did not detect OTHER type\n");
			return -1;
		}
	}
	return -1;
}
EXPORT_SYMBOL(asus_force_reset_as_CDP);

static void asus_bc_1_2_flow(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work, struct smbchg_chip,
		asus_bc_1_2_det_work.work);
	int rc, type;
	u8 val, reg;
	char *usb_type_name = "null";
	static char *ASUSEvt_cable_type_text;
	...
	chip->asus_dual_flag = Dual_None;
	chip->asus_hvdcp_flag = 0;
	/* read cable type again */
	rc = smbchg_read(chip, &reg, chip->misc_base + IDEV_STS, 1);
	if (rc < 0) {
		pr_err("Couldn't read status 5 rc = %d\n", rc);
		usb_type_name = "Other";
	}
	type = get_type(reg);
	usb_type_name = get_usb_type_name(type);

	pr_info("Detecting %s type !!\n", (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
									chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3 ) ? "HVDCP": usb_type_name);
	switch (chip->usb_supply_type) {
		case POWER_SUPPLY_TYPE_USB:
			ASUSEvt_cable_type_text = "USB";
			break;
		case POWER_SUPPLY_TYPE_USB_CDP:
		case POWER_SUPPLY_TYPE_USB_DCP:
		case POWER_SUPPLY_TYPE_USB_HVDCP:
		case POWER_SUPPLY_TYPE_USB_HVDCP_3:
			ASUSEvt_cable_type_text = "ASUS AC";
			break;
		default:
			ASUSEvt_cable_type_text = "UNKNOWN";
			break;
	}
	ASUSEvtlog("[USB] set_chg_mode: %s\n", ASUSEvt_cable_type_text);
	...
	switch (chip->usb_supply_type) {
		case POWER_SUPPLY_TYPE_USB:
			...
			if (chip->asus_typec_chg > TYPEC_PD_NO_READY_CHARGER)
				goto DFP_setting;
#endif
			asus_read_ICL_mode(chip, &val);
			if (val != 0x2) {
				// USBIN_MODE_CHG = USB 500 Mode Current Level, 0x1340 = 0x06
				rc = smbchg_masked_write(chip, chip->usb_chgpth_base + CMD_IL,
					SMB_MASK(6, 0), 0x6);
				if (rc < 0)
					pr_err("set USBIN =USB 500 Mode failed\n");
...
DFP_setting:
...
#endif
				pr_info("setting usb input limit current: 1450 mA\n");
				// USBIN_MODE_CHG = HC Mode Current Level, 0x1340 = 0x05
				// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_1400MA, 0x13F2 = 0xE
				// re-run AICL
				asus_set_high_usb_chg_current(chip, 1400);
		...
		case POWER_SUPPLY_TYPE_USB_HVDCP_3:
			if (type == 1) {
				...
				// SMBCHGL_USB_USBIN_IL_CFG = USBIN_IL_900MA, 0x13F2 = 0x09
				// re-run AICL
				pr_info("setting usb input limit current: 900 mA\n");
				asus_set_high_usb_chg_current(chip, 900);
#ifdef PD_CHARGING_DRIVER_SUPPORT
				// check DFP type
				if (chip->asus_typec_chg > TYPEC_PD_NO_READY_CHARGER)
				...
				if (is_hvdcp_present(chip)) {
					...
					if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
						...
						chip->asus_hvdcp_flag = 1;
						if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
							asus_auto_pulse_back_to_5V(chip);
					}
				} else {
					chip->asus_hvdcp_flag = 0;
					if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
						chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
						pr_err("***** Error: hvdcp gone...retry\n");
						return;
					}
				}
				asus_recheck_dcp_setting(chip);
				return;
			}
			break;
		default:
			pr_err("Unknown charger type. 0x1608: 0x%x. type: %d\n", reg, chip->usb_supply_type);
			break;
	}
	// JEITA
	asus_JEITA_rule(chip);
...
static void asus_thermal_policy_func(struct work_struct *work) {
	struct smbchg_chip *chip = container_of(work, struct smbchg_chip,
		asus_thermal_policy_work.work);
	asus_thermal_policy(chip);
}

static void asus_JEITA_check_func(struct work_struct *work) {
	struct smbchg_chip *chip = container_of(work, struct smbchg_chip,
		asus_JEITA_check_work.work);
#ifdef PD_CHARGING_DRIVER_SUPPORT
	if (is_usb_present(chip) && chip->asus_typec_chg == TYPEC_PD_5V_CHARGER
				&& !asus_is_low_battery())
			usbpd_jump_voltage();
#endif

	if (is_usb_present(chip) && chip->asus_dual_flag == Dual_None)
		asus_recheck_dcp_setting(chip);
	else
		asus_JEITA_rule(chip);
}

void asus_force_shipping_mode(void)
{
	struct smbchg_chip *chip;
	u8 reg = 0xA5;
	int rc;
	rc = qpnp_pon_system_pwr_off(PON_POWER_OFF_SHUTDOWN);
	if (!rc) {
		pr_err("failed to set PON_PS_HOLD_RESET_CTL to shutdown, retry once...\n");
		qpnp_pon_system_pwr_off(PON_POWER_OFF_SHUTDOWN);
	}
	chip = g_smbchg_chip;
	smbchg_write(chip, &reg, 0x12D0, 1);
	reg = 0x0;
	smbchg_write(chip, &reg, 0x1240, 1);
	//smbchg_sec_masked_write(chip, chip->bat_if_base + 0x40, 0xFF, 0x0);
}
EXPORT_SYMBOL(asus_force_shipping_mode);
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	smbchg_wq = create_singlethread_workqueue("smbchg_wq");
	ASUS_CHG_TYPE = "UNDEFINED";
	chip->asus_is_first_run = false;
	chip->asus_is_otg = false;
	chip->asus_cable_online = false;
	...
	chip->asus_hvdcp_flag = 0;
	chip->asus_hvdcp_rerun_flag = 0;
	chip->asus_batt_last_state = UNKNOWN_STATE;
#ifdef PD_CHARGING_DRIVER_SUPPORT
	chip->asus_typec_chg = TYPEC_UNKNOWN_CHARGER;
	chip->asus_cc_wa_flag = 0;
	chip->asus_pd_volt = 0;
	chip->asus_pd_curr = 0;
#endif
	mutex_init(&chip->asus_jeita_setting_lock);
	wake_lock_init(&chip->asus_jeita_setting_wake_lock,
			WAKE_LOCK_SUSPEND, "asus_jeita_setting_wake_lock");
	wake_lock_init(&chip->asus_adapter_detect_wake_lock,
			WAKE_LOCK_SUSPEND, "asus_adapter_detect_wake_lock");
	wake_lock_init(&chip->asus_update_battery_wake_lock,
			WAKE_LOCK_SUSPEND, "asus_update_battery_wake_lock");
	INIT_DELAYED_WORK(&chip->asus_bc_1_2_det_work, asus_bc_1_2_flow);
	INIT_DELAYED_WORK(&chip->asus_JEITA_check_work, asus_JEITA_check_func);
	INIT_DELAYED_WORK(&chip->asus_thermal_policy_work, asus_thermal_policy_func);
	INIT_DELAYED_WORK(&chip->asus_adapter_detect_work, asus_adapter_detect_func);
	INIT_DELAYED_WORK(&chip->asus_update_battery_work, asus_update_battery_func);
...
*/
	...
	if (gpio_is_valid(chip->adcpwren_gpio)) {
                rc = gpio_request(chip->adcpwren_gpio, "adcpwren_gpio");
                if (rc) {
                        pr_err("requset adcpwren_gpio failed\n");
                        return rc;
                }
                rc = gpio_direction_output(chip->adcpwren_gpio, 0);
                if (rc) {
                        pr_err("adcpwren_gpio set direction failed\n");
                        return rc;
                }
        }
	if (gpio_is_valid(chip->adc_sw_en_gpio)) {
                rc = gpio_request(chip->adc_sw_en_gpio, "adc_sw_en_gpio");
                if (rc) {
                        pr_err("requset adc_sw_en_gpio failed\n");
                        return rc;
                }
                rc = gpio_direction_output(chip->adc_sw_en_gpio, 0);
                if (rc) {
                        pr_err("adc_sw_en_gpio set direction failed\n");
                        return rc;
                }
        }
	asus_power_on_settings(chip);
	dump_regs(chip);
...
static int asus_smbchg_suspend(struct device *dev)
{
	struct smbchg_chip *chip = dev_get_drvdata(dev);
...
static int asus_smbchg_resume(struct device *dev)
{
	struct smbchg_chip *chip = dev_get_drvdata(dev);

	queue_delayed_work(smbchg_wq, &chip->asus_update_battery_work, 0);
	queue_delayed_work(smbchg_wq, &chip->asus_thermal_policy_work, msecs_to_jiffies(10000));
	queue_delayed_work(smbchg_wq, &chip->asus_JEITA_check_work, msecs_to_jiffies(60000));
	if (g_charger_mode)
		asus_smb1351_enable_wdt(true);
	return 0;
}
