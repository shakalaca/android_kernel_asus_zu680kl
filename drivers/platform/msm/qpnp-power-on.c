...
static irqreturn_t qpnp_kpdpwr_irq(int irq, void *_pon)
{
	...
	if (pon->pon_cfg->old_state) {
		...
		wake_lock_timeout(&pon->press_lock, HZ / 2);
		if (g_charger_mode &&
		    !wake_lock_active(&pon->press_lock_with_chargermode)) {
			pr_info("[%s] hold wakelock for charger mode\n",
							pon->pon_input->name);
		...
		if (g_charger_mode &&
		    wake_lock_active(&pon->press_lock_with_chargermode)) {
			pr_info("[%s] wake unlock for charger mode\n",
							pon->pon_input->name);
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	wake_lock_init(&pon->press_lock, WAKE_LOCK_SUSPEND, "pwr_btn_press");
	wake_lock_init(&pon->press_lock_with_chargermode, WAKE_LOCK_SUSPEND,
					"pwr_btn_press_with_chragermode");
