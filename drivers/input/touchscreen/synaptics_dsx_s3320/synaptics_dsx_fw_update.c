...
static enum flash_area fwu_go_nogo(void)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			...
			__func__, fwu->config_id[0], fwu->config_id[1],
			fwu->config_id[2], fwu->config_id[3]);

	dev_info(rmi4_data->pdev->dev.parent,
			"%s: Image config ID = %02x%02x%02x%02x\n",
			__func__, fwu->img.ui_config.data[0],
	...
	if (g_ftm_mode) {
		dev_info(rmi4_data->pdev->dev.parent,
			"%s: check special build FW\n",
				__func__);
		if (fwu->config_id[3] == 0xff) {
			dev_info(rmi4_data->pdev->dev.parent,
			"%s: It's speical build FW, skip fw update\n",
				__func__);
			flash_area = NONE;
			goto exit;
		}
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	if (!do_once) {
		rmi4_data->auto_update_finish = true;
		return;
	}
	do_once = 0;
